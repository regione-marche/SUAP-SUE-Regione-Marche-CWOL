...
class cwbLibAllegatiBtaDurcal extends cwbLibAllegati implements cwbLibAllegatiInterface {
    ...
    protected function caricaAllegatoCustom($chiaveTestata, $riga, $datiAggiuntivi = array(), $startedTransaction = false, $modVis = false) {
        $filtri = array(
            'PROG_DURC' => $chiaveTestata['PROG_DURC'],
            'RIGA_DURC' => $riga
        );
        //Lettura dal Db del documento
        $data = $this->libDB_BTA->leggiBtaDurcal($filtri, false);
    ...
    protected function caricaAllegatiCustom($chiave = array()) {
        $filtri = array(
            'PROG_DURC' => $chiave['PROG_DURC']
        );
        $data = $this->libDB_BTA->leggiBtaDurcal($filtri, true);
    ...
    public function allegatoToAllegatoModel($allegato = array()) {
        if (!empty($allegato)) {
            ...
            $rowModel["CHIAVE_ESTERNA"] = array(
                'PROG_DURC' => $allegato['PROG_DURC']
            );
            $rowModel["RIGA"] = $allegato["RIGA_DURC"];
            $rowModel["DESCRIZIONE"] = $allegato["DES_ALLEG"];
            $rowModel["CORPO"] = $allegato["TESTOATTO"];
//            $rowModel["NATURA"] = $allegato["QUALIFICA"];
    ...
    public function allegatoModelToAllegato(&$allegato = array(), $rowModel = array()) {
        $allegato["PROG_DURC"] = $rowModel["CHIAVE_ESTERNA"]["PROG_DURC"];
        $allegato["RIGA_DURC"] = $rowModel["RIGA"];
        $allegato["DES_ALLEG"] = $rowModel["DESCRIZIONE"];
        $allegato["TESTOATTO"] = $rowModel["CORPO"];
//        $allegato["QUALIFICA"] = $rowModel["NATURA"];
    ...
    public function caricaDatiHeader($chiaveTestata) {
        return $this->libDB_BTA->leggiGeneric('BTA_DURC', $chiaveTestata, false);   
    }
