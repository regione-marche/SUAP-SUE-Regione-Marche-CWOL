...
include_once ITA_BASE_PATH . '/apps/CityBase/cwbPagoPaMPay.class.php';
...
function cwbBgeAgidConfMpay() {
    $cwbBgeAgidConfMpay = new cwbBgeAgidConfMpay();
    $cwbBgeAgidConfMpay->parseEvent();
    return;
}

class cwbBgeAgidConfMpay extends cwbBpaGenTab {
    ...
    function initVars() {
        ...
        $this->pagoPa = new itaPagoPa(itaPagoPa::MPAY_TYPE);
        $this->skipAuth = true;
    ...
    protected function customParseEvent() {
        switch ($_POST['event']) {
            case 'onClick':
                switch ($_POST['id']) {
                    case $this->nameForm . '_BGE_AGID_CONF_MPAY[SFTPFILEKEY]_upld':
    ...
    protected function postApriForm() {
        ...
        $this->initComboIUV();
        Out::valore($this->nameForm . '_BGE_AGID_CONF_MPAY[GENERAIUV]', 0);
        Out::valore($this->nameForm . '_BGE_AGID_CONF_MPAY[FILLER]', '$');
        Out::disableField($this->nameForm . '_GENERAIUV');
        $configurazione = $this->libDB->leggiBgeAgidConfMpay();
        ...
        Out::setFocus("", $this->nameForm . '_BGE_AGID_CONF_MPAY[AUXDIGIT]');
        // serve per attivare il componenre ita-edit-upload
        Out::codice("pluploadActivate('" . $this->nameForm . "_BGE_AGID_CONF_MPAY[SFTPFILEKEY]_upld_uploader');");

        if ($configurazione['SFTPFILEKEY']) {
            Out::valore($this->nameForm . '_BGE_AGID_CONF_MPAY[SFTPFILEKEY]', "certificato.pem");
        } else {
            Out::valore($this->nameForm . '_BGE_AGID_CONF_MPAY[SFTPFILEKEY]', "");
        }
    }
    ...
    protected function elaboraCurrentRecord($operation) {
        if ($operation !== itaModelService::OPERATION_DELETE) {
            ...
            if ($_POST[$this->nameForm . '_BGE_AGID_CONF_MPAY']['SFTPFILEKEY']) {
    ...
    private function caricaCertificato() {
        ...
        Out::valore($this->nameForm . '_BGE_AGID_CONF_MPAY[SFTPFILEKEY]', $origFile);
    }
