...
/**
 *
 * Classe per la gestione dell'intermediario alto adige riscossioni
 * 
 */
class cwbPagoPaAltoAdigeRisco extends cwbPagoPaMaster {
    ...
    protected function customGeneraBollettino($params) {
        ...
        if ($params['CDS'] == true) {
            return $this->generaBollettinoCDS($agidScadenza);
        }
    ...
    public function getCustomConfIntermediario() {
        return $this->getLibDB_BGE()->leggiBgeAgidConfAltoAdigeRisco(array());
    }
    ...
    protected function getCodiceSegregazione() {
        $filtri['INTERMEDIARIO'] = itaPagoPa::ALTOADIGE_RISCO_TYPE;
    ...
    protected function leggiScadenzePerPubblicazioni($progkeytabScadenze = null, $stato = null, $page = null) {
        ...
        if ($page === null) {
            $scadenzePerPubbli = $this->getLibDB_BTA()->leggiBgeAgidScadenzePerPubblicazioniAltoAdigeRisco($filtri);
        } else {
            $scadenzePerPubbli = $this->getLibDB_BTA()->leggiBgeAgidScadenzePerPubblicazioniAltoAdigeRiscoBlocchi($filtri, $page, $this->customPaginatorSize());
        }
...
//   $risposta = $this->caricaDelegheFeedZZ($scadenzePerPubbli);
    ...
    protected function customEseguiPagamento($scadenza, $urlReturn, $redirectVerticale = 0) {
        ...
        //qui va a leggere i parametri nella tabella BGE_AGID_CONF_AAR
        $parametriAAR = $this->getCustomConfIntermediario();
        $T = date('YmdHi');
        ...
        $CodiceUtente = $parametriAAR['CODUTE'];
        $CodiceEnte = $parametriAAR['CODENTECRED'];
        ...
        $importo = intval($scadenza['IMPDAPAGTO'] * 100);
        $CF = $soggetto['CODFISCALE'] != '' ? $soggetto['CODFISCALE'] : $soggetto['PARTIVA'];

        $BD = '<PaymentRequest>' .
                '<PortaleID>' . $parametriAAR['PORTALEID'] . '</PortaleID>' .
...
//                '<CodiceUtenteBeneficiario>' . $parametriAAR['CODUTEBENEFICIARIO'] . '</CodiceUtenteBeneficiario>' .
//                '<CodiceEnteBeneficiario>' . $parametriAAR['CODENTEBENEFICIARIO'] . '</CodiceEnteBeneficiario>' .
...
//                '</AccountingData>' .
        ...
        $H = md5($parametriAAR['ENCRYPTIV'] . $BD . $parametriAAR['ENCRYPTKEY'] . $T);

        //preparazione del Bi = $T . $C . base64_encode($BD) . $H;
        $Bi = '<Buffer>' .
                '<TagOrario>' . $T . '</TagOrario>' .
                '<CodicePortale>' . $parametriAAR['PORTALEID'] . '</CodicePortale >' .
        ...
        $UrlMPAY = $parametriAAR['URL_PAGAM_WS'];
...
//        $debug = $itaRestClient->getDebug();
        ...
        $H = md5($parametriAAR['ENCRYPTIV'] . $rid . $parametriAAR['ENCRYPTKEY'] . $T);
        $Bi = '<Buffer>' .
                '<TagOrario>' . $T . '</TagOrario>' .
                '<CodicePortale>' . $parametriAAR['PORTALEID'] . '</CodicePortale>' .
        ...
        $UrlMPAYCart = $parametriAAR['URL_PAGAM_REDIRECT'];
...
//    }
            ...
            foreach ($listOfFiles as $value) {
                ...
                try {
                    ...
                    if ($zipFile) {
                        ...
                        $ricezione = array(
                            "TIPO" => 15,
                            "INTERMEDIARIO" => itaPagoPa::ALTOADIGE_RISCO_TYPE,
                            "CODSERVIZIO" => 0,
                            "DATARIC" => $dataFile,
                            "IDINV" => null,
                        );
                        ...
                        foreach ($arrayTxt as $key => $value) {
                            ...
                            if ($scadenza) {
                                ...
                                $this->inserisciBgeAgidRiscoAltoAdigeRisco($risco_mpay);
                            } else {
                                $errore = true;
                                $risultato = false;
    ...
    public function inserisciBgeAgidRiscoAltoAdigeRisco($toInsert, $startedTransaction = true) {
        return $this->insertUpdateRecord($toInsert, 'BGE_AGID_RISCO_ALTOADIGE', true, $startedTransaction);
    }
...
//        $UrlPID = 'http://payertest.regione.marche.it/mpay/cart/extS2SPID.do';
    ...
    protected function customAgidRiscoSpecifica($progkeytabScade) {
        ...
        return $this->getLibDB_BGE()->leggiBgeAgidRiscoAAR(array("PROGKEYTAB" => $progkeytabScade), false);
    }

//    private function generaBollettinoCDS($agidScadenza1, $agidScadenza2, $agidScadenza3 = array()) {
    private function generaBollettinoCDS($agidScadenza) {
        if (!$agidScadenza) {
            $this->setErrMessage("Scadenza non definita");
            return false;
        }
        ...
        /*
         * @TODO: trovare tutte le scadenze del verbale e restituire il bollettino relativo
         */
        $filtri = array('PROGCITYSCA' => $iuv);
        $agidScadenza = $cwbPagoPaAltoAdigeRisco->getLibDB_BGE()->leggiBgeAgidScadenze($filtri, false);

        $infoAggiuntive = $this->getLibDB_BGE()->leggiBgeAgidScainfo(array("IDSCADENZA" => $agidScadenza['PROGKEYTAB']));
        $params = array(
            'IDENTIFICATIVOVERBALE' => $infoAggiuntive['IDENTIFICATIVOVERBALE'],
            'DATAVERBALE' => $infoAggiuntive['DATAVERBALE'],
            'TARGAVEICOLO' => strtoupper($infoAggiuntive['TARGAVEICOLO'])
        );
        $scadenze_tab = $cwbPagoPaAltoAdigeRisco->leggiScadenzaPerInfoAggiuntive($params, true);
        /*
         * 30
         * $ProgCitySca = "CDS_" . $Contrav_rec['VERBALE'] . "/" . $Contrav_rec['ANNO'] . "_" . $tipo;
         */


        $verbale = array();
        $soggetto = array();
        //cerco il verbale tramite ws
        include_once ITA_BASE_PATH . '/apps/Sviluppo/devLib.class.php';
        $devLib = new devLib();
        $paramTmp = $devLib->getEnv_config("ITALCDSWSCONNECTION");
        $clientParam = array();
        foreach ($paramTmp as $arrayParametro) {
            $clientParam[$arrayParametro['CHIAVE']] = $arrayParametro['CONFIG'];
        }
        //chiamata al client ws
        include_once (ITA_BASE_PATH . '/apps/Cds/cdsWsManager.class.php');
        $cdsWsManager = cdsWsManager::getInstance($clientParam);
        $param_rec = array();
        $param_rec['verbale'] = $infoAggiuntive['IDENTIFICATIVOVERBALE'];
        $param_rec['anno'] = substr($infoAggiuntive['DATAVERBALE'], 0, 4);
        $param_rec['flagFoto'] = false;
        $risultato = $cdsWsManager->GetInfoVerbale($param_rec);
        $verbale = $risultato['RetValue'];
        file_put_contents("/tmp/risultato_infoVerbale", print_r($risultato, true));
        if ($risultato['Status'] == "-1") {
            $this->setErrMessage("Verbale non trovato");
            return false;
        }
        if (is_array($risultato['RetValue']['soggetti'])) {
            foreach ($risultato['RetValue']['soggetti'] as $sogg_rec) {
                //sull'array sono ordinati per proprietario, contravventore, altri nominativi. Torno il proprietario o al limite altri nominativi
                if ($sogg_rec['tipoSoggetto'] == 'PROPRIETARIO') {
                    $soggetto['COGNOME'] = $sogg_rec['denominazione'];
                    $soggetto['CODFISCALE'] = $sogg_rec['codiceFiscale'];
                    $soggetto['INDIRIZZORESID'] = $sogg_rec['via'];
                    $soggetto['COMUNERESID'] = $sogg_rec['citta'];
                }
                if ($sogg_rec['tipoSoggetto'] == 'CONTRAVVENTORE') {
                    $soggetto['COGNOME'] = $sogg_rec['denominazione'];
                    $soggetto['CODFISCALE'] = $sogg_rec['codiceFiscale'];
                    $soggetto['INDIRIZZORESID'] = $sogg_rec['via'];
                    $soggetto['COMUNERESID'] = $sogg_rec['citta'];
        ...
        switch (count($scadenze_tab)) {
            case 1:
                return $this->generaBollettinoCDS1($scadenze_tab[0], $soggetto);
            case 2:
                return $this->generaBollettinoCDS1($scadenze_tab[0], $scadenze_tab[1], $soggetto);
            case 3:
                return $this->generaBollettinoCDS1($scadenze_tab[0], $scadenze_tab[1], $scadenze_tab[3], $soggetto);
            default:
                $this->setErrMessage("Scadenze non trovate per il verbale");
                return false;
        }
    ...
    private function generaBollettinoCDS1($agidScadenza1, $soggetto) {
        
    }

    private function generaBollettinoCDS2($agidScadenza1, $agidScadenza2, $soggetto) {
        
    }

    private function generaBollettinoCDS3($agidScadenza1, $agidScadenza2, $agidScadenza3, $soggetto) {
