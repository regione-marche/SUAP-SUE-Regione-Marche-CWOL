                ...
                switch ($_POST['id']) {
                    case $this->gridElencoCapitoli:
                        switch ($_POST['colName']) {
                            case 'IMPEGNI':
                                $indice = $_POST['rowid'];
                                $capitolo = $this->capitoli[$indice];
                                if ($indice >= 0) {

                                    
                                    $model = 'cwfMastroSintetico';
                                    $formObj = cwbLib::apriFinestra($model, $this->nameFormOrig, '', '', array(), $this->nameForm);
                                    $formObj->ANNO_ESE = $capitolo['ANNO_ESE'];      // Anno da Consultare
                                    $formObj->E_S = $capitolo['E_S'];                // E-ntrata S-pesa
                                    $formObj->FILTRI = array('PROGKEYVB' => $capitolo['PROGKEYVB']); // Chiave Voce di Bilancio da Visualizzare
                                    $formObj->parseEvent();
