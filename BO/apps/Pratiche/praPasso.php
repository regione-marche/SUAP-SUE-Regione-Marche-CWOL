...
class praPasso extends itaModel {
    ...
    public $COMUNI_DB;
    public $ITW_DB;
    public $nameForm = "praPasso";
    public $divGes = "praPasso_divGestione";
    public $divAllegatiCom = "praPasso_divAllegatiCom";
    public $gridAllegati = "praPasso_gridAllegati";
    public $gridAllCom = "praPasso_gridAllCom";
    public $gridNote = "praPasso_gridNote";
    public $gridAltriDestinatari = "praPasso_gridAltriDestinatari";
                ...
                switch ($_POST['id']) {
                    case $this->nameForm . '_Aggiungi':
                        ...
                        try {
                            ...
                            $tableDef = ItaDB::DBGetTableObject($this->PRAM_DB, 'PROPAS');
                            $propas_rec = $this->manageAudit(itaModelService::OPERATION_INSERT, $tableDef, $propas_rec, App::$utente->getKey('nomeUtente'));
                            ...
                            $insert_Info = 'Oggetto: Inserimento passo con chiave ' . $propas_rec['PROPAK'] . " e seq " . $propas_rec['PROSEQ'];
                            if ($this->insertRecord($this->PRAM_DB, 'PROPAS', $propas_rec, $insert_Info)) {
                                ...
                                if ($this->daMail["protocolla"] || $this->duplica) {
                                    $this->AggiornaPartenzaDaPost();
                                    $this->AggiornaArrivo();
                                }
                                if ($this->docCommercio) {
                                    $wcoLib = new wcoLib();
                                    ...
                                    if (!$this->praLib->AggiungiDocumentoCommercio($propas_rec, $this->docCommercio)) {
                                        Out::msgStop("Attenzione!!!", "Errore importazione allegato");
                                        break;
                                    }
                                    //$wcoLib = new wcoLib();
                                    $compro_rec = $wcoLib->getCompro($this->docCommercio['ROWID'], "rowid");
                                    $compro_rec['PROINV'] = date("Ymd");
                                    $compro_rec['PROPAK'] = $propas_rec['PROPAK'];
                                    $update_Info = 'Oggetto: Aggiorno Allegato commercio: ' . $this->docCommercio['PRODOC'];
                                    $this->updateRecord($wcoLib->getCOMMDB(), 'COMPRO', $compro_rec, $update_Info);

                                    /*
                                     * Lega pratica SUAP
                                     */
                                    $comsua_rec = $wcoLib->GetComsua($compro_rec['PROCOD']);
                                    if (!$comsua_rec) {
                                        $proges_rec = $this->praLib->GetProges($propas_rec['PRONUM']);
                                        if (!$wcoLib->CollegaPraticaSuap($compro_rec['PROCOD'], $proges_rec, $propas_rec['PROPAK'])) {
                                            Out::msgStop("Attenzione!!!!", "Errore collegamento a pratica suap n. " . $proges_rec['GESNUM']);
                                            break;
                                        }
                                    }
                                }
                                ...
                                /*
                                 * Inserisco Record ANAPRO se il parametro è attivo
                                 * Commentato perchè lo fa al Dettaglio
                                 */
//                                if ($this->flagAssegnazioniPasso) {
//                                    $retInsertAnapro = $this->inserisciAnapro($propas_rec['PROPAK']);
...
//                                }
                                 ...
                                 */
                                $errMsg = $this->praLib->sincCalendar("PASSO", $propas_rec['PROPAK'], $propas_rec['PRODSC'], $_POST[$this->nameForm . "_Calendario"]);
                                if ($errMsg) {
                                ...
                                if (is_array($this->daMail)) {
                                    if (!$this->praLib->setClasseCaricatoPasso($this->daMail['archivio'], $this->daMail['IDMAIL'], $this->currGesnum, $propas_rec['PROPAK'])) {
                                        Out::msgStop("Errore", $this->praLib->getErrMessage());
                                    }
                                    $this->datiForm = array(
                                    ...
                                    $propas_rec = $this->praLib->GetPropas($propas_rec['PROPAK'], 'propak');
                                    ...
                                    //if (!$this->inviaNotificaProtocolloArrivo($propas_rec, $titolo, $testo)) {
...
//                                    }
                    ...
                    case $this->nameForm . '_Aggiorna':
                        ...
                        /*
                         * Controllo se il passo è aperto se no propongo l'apertura
                         *
                         */
                        $propas_rec = $this->praLib->GetPropas($this->keyPasso, "propak");
...
//                                }
                            ...
                            $this->Dettaglio($this->keyPasso);
                            Out::msgBlock($this->nameForm, 500, true, "Passo Aggiornato Correttamente");
                        }
                        break;
...
//                        );
                    ...
                    case $this->nameForm . '_PROPAS[PRODTP]_butt':
                        ...
                        break;
//                    case $this->nameForm . '_DESC_DESTINATARIO_butt':
//                        proRic::proRicAnamed($this->nameForm, $where, 'proAnamed', '2');
//                        break;
                    case $this->nameForm . '_DESC_MITTENTE_butt':
                        if ($_POST[$this->nameForm . "_EMAIL_MITTENTE"]) {
                            ...
                            praRic::praRicMittente($mittente_tab, $this->nameForm, $tipo);
                        } else {
                            proRic::proRicAnamed($this->nameForm, $where, 'proAnamed', '3');
                        }
//                        $where = " WHERE MEDEMA = '" . $_POST[$this->nameForm . "_EMAIL_MITTENTE"] . "'";
//                        proRic::proRicAnamed($this->nameForm, $where, 'proAnamed', '3');
                        break;
                    case $this->nameForm . '_PROPAS[PROCLT]_butt':
                        praRic::praRicPraclt("praPasso", "RICERCA Tipo Passo", "returnPraclt");
                        break;
                    case $this->nameForm . '_PROPAS[PRORPA]_butt':
                        praRic::praRicAnanom($this->PRAM_DB, "praPasso", "RICERCA dipendenti", "", "returnUnires");
                        break;
                    case $this->nameForm . '_PROPAS[PROSET]_butt':
                        praRic::praRicAnauni("praPasso", "RICERCA SETTORE", "returnSettore");
                        break;
                    case $this->nameForm . '_PROPAS[PROSER]_butt':
                        praRic::praRicAnaSer($this->PRAM_DB, "praPasso", "RICERCA SERVIZIO", "AND UNISET = '" . $_POST[$this->nameForm . '_PROPAS']['PROSET'] . "'", "returnServizio");
                        break;
                    ...
                    case $this->nameForm . '_EMAIL_MITTENTE_butt':
                        ...
                        $ret = proRic::proRicTabdagMail($this->nameForm, "ANAMED", $Anamed_rec['ROWID'], "", "Il Mittente ha più Mail. Sceglierne una");
                        if ($ret) {
                    ...
                    case $this->nameForm . '_SEQANTECEDENTE_butt':
                        $where = " AND PRONUM='$this->currGesnum' AND PROFIN='' AND PROPUB='' AND PROKPRE='' AND PROSEQ<>" . $_POST[$this->nameForm . '_PROPAS']['PROSEQ'];
                        praRic::praRicPropas($this->nameForm, $where, 'ANTECEDENTE', "Seleziona il passo da collegare al seguente", true);
                        break;
                    ...
                    case $this->nameForm . '_BloccaAllegatiProt':
                        ...
                        if ($arrayDoc) {
                            ...
                            praRic::ricAllegatiWs($this->PRAM_DB, $arrayDocWs, $this->nameForm, 'returnAggiungiAllegatiWs', $msgInfo);
                        }
                        break;
                    case $this->nameForm . '_BloccaAllegatiArr':
                        ...
                        if ($arrayDoc) {
                            ...
                            praRic::ricAllegatiWs($this->PRAM_DB, $arrayDocWs, $this->nameForm, 'returnAggiungiAllegatiWsArr');
                        }
                        break;
                    ...
                    case $this->nameForm . '_ConfermaCancAlle':
                        ...
                        if (array_key_exists($this->rowidAppoggio, $this->passAlle) == true) {
                            ...
                            if (in_array(strtolower($ext), array('xls', 'xlsx'))) {
                                $fileElaborato = $this->praLibAllegati->getFilenameFoglioElaborato($this->passAlle[$this->rowidAppoggio]['FILEPATH']);
                                $fileDefinitivo = $this->praLibAllegati->getFilenameFoglioElaborato($this->passAlle[$this->rowidAppoggio]['FILEPATH'], true);
...
//
                        ...
                        //Out::msgInfo("Codice Cancella", $dest['CODICE']);
                        // Cancella il destinatario se ha associato qualche allegato del passo.
                        $this->cancellaDestinatarioAllegatoPasso($dest['CODICE']);

                        unset($this->destinatari[$this->rowidAppoggio]);
                        $this->destinatari = array_values($this->destinatari);
                        $this->CaricaGriglia($this->gridAltriDestinatari, $this->destinatari, "1", "100");
                        $this->attivaInvioCart();
...
//                        }
                    ...
                    case $this->nameForm . '_Chiudi':
                        ...
                        $array_btn = array();
                        $array_btn['Annulla'] = array('id' => $this->nameForm . '_AnnullaChiudiPasso', 'model' => $this->nameForm, 'shortCut' => "f5");
                        $array_btn['Chiudi'] = array('id' => $this->nameForm . '_ChiudiPasso', 'model' => $this->nameForm, 'shortCut' => "f5");
                        if ($propas_rec['PROKPRE'] == '' && $nextPropas_rec) {
                            $array_btn['Chiudi e attiva passo successivo'] = array('id' => $this->nameForm . '_Chiudi_E_Act', 'model' => $this->nameForm, 'shortCut' => "f5");
                        }
                        $array_btn['Chiudi e Apri altro passo'] = array('id' => $this->nameForm . '_Chiudi_E_New', 'model' => $this->nameForm, 'shortCut' => "f8");
                        Out::msgQuestion("Nuovo Passo", "Chiudere il passo inserendo la data odierna e opzionalmente attivare un altro passo?", $array_btn);
                        break;
                    ...
                    case $this->nameForm . '_Cancella':
                        ...
                        if ($propas_rec['PRODATEPUBART'] && $propas_rec['PRODATEPUBART'] != '0000-00-00') {
                            Out::msgInfo("Cancellazione Passo", "Il passo ha un articolo pubblicato. <br>Salvare la spubblicazione dell'articolo prima di effettuare la cancellazione.");
                            break;
                        }
...
//
                    ...
                    case $this->nameForm . '_ConfermaCancella':
                        ...
                        if (!$praFrontOfficeManager->ripristinaIntegrazione($propas_rec['PRONUM'], $propas_rec['PROPAK'])) {
                            $this->setErrCode($praFrontOfficeManager->getErrCode());
                            $this->setErrMessage($praFrontOfficeManager->getErrMessage());
                            return false;
                        }

                        // Si cancella eventuale pubblicaiozne articolo in FO Remoto
                        $this->gestPubbArticoloFORemoto("Delete");
                        ...
                        // Cancella Directory e gli allegati collegati al passo
                        ...
                        $pramail_rec = $this->praLib->GetPramail($this->keyPasso, "propak");
                        if ($pramail_rec) {
                            $delete_Info = 'Oggetto: Cancellazione record mail passo: ' . $this->keyPasso;
                            $this->deleteRecord($this->PRAM_DB, 'PRAMAIL', $pramail_rec['ROWID'], $delete_Info);
                        }
...
//
                    ...
                    case $this->nameForm . '_Risposta':
                        ...
                        $model();
                        break;
                    case $this->nameForm . '_InviaCart':

                        //$valori['PRACOM'] = $_POST[$this->nameForm . '_PRACOM'];
                        $valori['Destinatari'] = $this->GetDestinatariCartInvio();
                        $arrayStimoli = array("notifica", "comunicazione", "richiestaIntegrazioni", "richiestaConformazioni", "diniego", "inoltroIntegrazione", "inoltroConformazione");
                        $isRichiedente = false;
                        $isEnte = false;

                        if ($valori['Destinatari']) {
                            ...
                            foreach ($valori['Destinatari'] as $enteDest) {
                                if ($enteDest['DESTCART'] == 'FACCT') {
                                    $isRichiedente = true;
                                } else {
                                    $isEnte = true;
                                }
                            ...
                            //                        Out::msgInfo("", "Richiedente -> " . $isRichiedente . "<br/> Ente Terzo --> " . $isEnte);

                            if (!$isRichiedente) {
                                if (($pos = array_search('richiestaIntegrazioni', $arrayStimoli)) !== false) {
                                    unset($arrayStimoli[$pos]);
                                }
                                if (($pos = array_search('richiestaConformazioni', $arrayStimoli)) !== false) {
                                    unset($arrayStimoli[$pos]);
                                }
                                if (($pos = array_search('diniego', $arrayStimoli)) !== false) {
                                    unset($arrayStimoli[$pos]);
                                }
                            }

                            if (!$isEnte) {
                                if (($pos = array_search('notifica', $arrayStimoli)) !== false) {
                                    unset($arrayStimoli[$pos]);
                                }
                                if (($pos = array_search('inoltroIntegrazione', $arrayStimoli)) !== false) {
                                    unset($arrayStimoli[$pos]);
                                }
                                if (($pos = array_search('inoltroConformazione', $arrayStimoli)) !== false) {
                                    unset($arrayStimoli[$pos]);
                                }
                            }


                            $valori['Stimoli'] = $arrayStimoli;
                            //$valori['Stimoli'] = array("notifica", "comunicazione", "richiestaIntegrazioni", "richiestaConformazioni", "diniego", "inoltroIntegrazione", "inoltroConformazione");

                            $valori['Oggetto'] = $_POST[$this->nameForm . '_PROPAS']['PRODPA'];
                            $valori['GgRisposta'] = $_POST[$this->nameForm . '_PRACOM']['COMGRS'];
                            ...
                            $_POST['returnEvent'] = 'returnInviaCart';
                            $_POST['event'] = 'openform';
                            $model = 'utiGestCartInvio';
                            itaLib::openForm($model);
                            $appRoute = App::getPath('appRoute.' . substr($model, 0, 3));
                            include_once App::getConf('modelBackEnd.php') . '/' . $appRoute . '/' . $model . '.php';
                            $model();
                    ...
                    case $this->nameForm . '_Invia':
                        ...
                        $proObject = proWsClientFactory::getInstance();
                        if (!$proObject) {
                            Out::msgStop("Invio Mail", "Errore inizializzazione driver protocollo");
                            return false;
                        }

                        if (is_object($proObject)) {
                            if (($proObject->getClientType() == proWsClientHelper::CLIENT_JIRIDE || $proObject->getClientType() == proWsClientHelper::CLIENT_PALEO4 || $proObject->getClientType() == proWsClientHelper::CLIENT_PALEO41 || $proObject->getClientType() == proWsClientHelper::CLIENT_ITALPROT) && $Metadati['DatiProtocollazione']['idMail']) {
                        ...
                        if ($anatsp_rec['TSPSENDREMOTEMAIL'] && ($proObject->getClientType() == proWsClientHelper::CLIENT_PALEO4 || $proObject->getClientType() == proWsClientHelper::CLIENT_PALEO41)) {
                            if ($_POST[$this->nameForm . '_PROTRIC_DESTINATARIO']) {
                                ...
                                Out::msgInfo("Attenzione", "Attivato invio mail con protocollo $this->tipoProtocollo.<br>Si prega prima di protocollare.");
                            }
                            break;
                        }
                    ...
                    case $this->nameForm . '_DaAltraPratica':
                        //praRic::praRicProges($this->nameForm, " AND GESNUM<>$this->currGesnum");
                        ...
                        $objModel->setReturnEvent('returnFromPraPasso');
                    ...
                    case $this->nameForm . '_TestoBase':
                        ...
                        if ($propas_rec['ROWID_DOC_CLASSIFICAZIONE'] != 0) {
                            ...
                            docRic::docRicDocumenti($this->nameForm, " WHERE CLASSIFICAZIONE = 'PRATICHE' AND (TIPO = 'XHTML' OR TIPO = 'DOCX') AND FUNZIONE != 'VARIABILE'");
                        } else {
                    ...
                    case $this->nameForm . '_ConfermaTestoBase':
                        ...
                        $this->passAlle = $this->praLib->caricaTestoBase($this->passAlle, $propas_rec['PROTBA']);
                        $this->ContaSizeAllegati($this->passAlle);
                        $this->CaricaGriglia($this->gridAllegati, $this->passAlle, '1', '100000');
                        $this->bloccoAllegatiRiservati();
                        $this->BloccaDescAllegati();
...
//$this->caricaTestoBase($propas_rec['PROTBA']);
                    ...
                    case $this->nameForm . '_ProtocollaPartenza':
                        ...
                        $this->tipoProtocollo = $this->praLib->getTipoProtocollo($this->currGesnum);
                        /*
                        ...
                        if (is_object($proObject)) {
                            switch ($proObject->getClientType()) {
                                case proWsClientHelper::CLIENT_ITALPROT:
                                    ...
                                    $arrayCampi = array(
                                        'label' => array('style' => "width:100px;", 'value' => $label),
                                ...
                                case proWsClientHelper::CLIENT_KIBERNETES:
                                    ...
                                    $arrayButton = array(
                                        'Protocolla Partenza' => array('id' => $this->nameForm . '_ConfermaProtPartenza', 'model' => $this->nameForm),
                                        'Documento Interno' => array('id' => $this->nameForm . '_ConfermaDocumentoFormale', 'model' => $this->nameForm)
                                    );
                                    break;
                            }
                        }
...
//
                    ...
                    case $this->nameForm . '_VediCollegamenti':
                        $propas_rec = $this->praLib->GetPropas($this->keyPasso, "propak");
                        praRic::praPassiAntecedenti($this->nameForm, $propas_rec, $this->PRAM_DB);
                        break;
                    case $this->nameForm . '_RimuoviAntecedente':
                        Out::valore($this->nameForm . '_SEQANTECEDENTE', '');
                        Out::valore($this->nameForm . '_DESCANTECEDENTE', '');
                        Out::valore($this->nameForm . '_PROPAS[PROKPRE]', '');
                        break;
...
//                            $this->praLib->VisualizzaFirme($filePathP7M, $fileOrig, $paramSegnatura, $this->passAlle[$this->rowidAppoggio]['ROWID']);
                    ...
                    case $this->nameForm . '_VisualizzaTestoBase':
                        $doc = $this->passAlle[$this->rowidAppoggio];
                        $this->praLibAllegati->ApriAllegatoTestoBase($this->nameForm, $doc, $this->currGesnum, $this->keyPasso);
                        break;
...
//                        break;
                    ...
                    case $this->nameForm . "_BloccaAllegatoDaMenu":
                        ...
                        praRic::praRicProtocolliPresenti($arrayProtPresenti, $this->nameForm);
                        break;
...
//
                    ...
                    case $this->nameForm . '_PROPAS[PRODOCTIPREG]_butt':
                        praRic::praRicAnadoctipreg($this->nameForm, "", " WHERE FL_ATTIVO = 1");
                        break;
...
//
                    ...
                    case $this->nameForm . "_CercaMittDest":
                        proRic::proRicAnamedMulti($this->nameForm, "", 'proAnamed', 'destinatari');
                        break;
                    ...
                    case $this->nameForm . "_Comint":
                        praRic::praRicAnades($this->nameForm, "", "WHERE DESNUM = '$this->currGesnum'", "true");
                        break;
                    ...
                    case $this->nameForm . "_CercaQualificati":
                        praRic::praRicAnaddo($this->nameForm, "", "Qualificati");
...
//                        $utiUploadDiag->parseEvent();
                        break;
                    case $this->nameForm . "_DestinatarioCart":

                        $sogg = array();
                        $sogg['ROWID'] = 0;
                        //$sogg['CODICE'] = $anaddo_rec['DDOCOD'];
                        $sogg['NOME'] = "<span style = \"color:orange;\">Richiedente CART</span>";
                        $sogg['FISCALE'] = "";
                        $sogg['INDIRIZZO'] = "";
                        $sogg['COMUNE'] = "";
                        $sogg['CAP'] = "";
                    ...
                    case $this->nameForm . '_ElaboraFoglioDiCalcolo':
                        ...
                        if (!$this->elaborazioneFoglioDiCalcolo($spreadsheetPath, $this->praLibAllegati->getFilenameFoglioElaborato($spreadsheetPath), true)) {
                            break;
                        }
                    ...
                    case $this->nameForm . '_DownloadXLSDef':
                        $docuRec = $this->passAlle[$this->rowidAppoggio];
                        $spreadsheetPath = $docuRec['FILEPATH'];
                        $definitivo = $this->praLibAllegati->getFilenameFoglioElaborato($spreadsheetPath, true);
                    ...
                    case $this->nameForm . "_VisualizzaFirmeCDS":
                        ...
                        break;
                    case $this->nameForm . '_PubblicaArticolo':
                        // Salvataggio Passo
                        if ($this->AggiornaRecord()) {
                            $this->CaricaAllegati($this->keyPasso);
                            $this->RegistraDestinatari();
                        ...
                        if ($this->gestPubbArticoloFORemoto("Insert")) {
                            Out::msgInfo("Attenzione", "Pubblicazione Articolo effettuata con successo");
                        }
                        $this->abilitaPubblicaArticolo();
            ...
            case 'onChange':
                switch ($_POST['id']) {
                    case $this->nameForm . '_PROPAS[PROPART]':
                        ...
                        $this->pubblicaArticolo($_POST[$this->nameForm . '_PROPAS']['PROPART'], $_POST[$this->nameForm . '_PROPAS']['PRODATEPUBART'], $_POST[$this->nameForm . '_PROPAS']['PROTIMEPUBART']);
                        break;
            ...
            case 'onBlur':
                switch ($_POST['id']) {
                    case $this->nameForm . '_PROPAS[PRORPA]':
                        ...
                        break;
                    case $this->nameForm . '_PROPAS[PROSET]':
                        $codice = $_POST[$this->nameForm . '_PROPAS']['PROSET'];
                        if (trim($codice) != "") {
                            $codice = str_repeat("0", 6 - strlen(trim($codice))) . trim($codice);
                            $Anauni_set = $this->praLib->getAnauni($codice);
                            Out::valore($this->nameForm . '_PROPAS[PROSET]', $codice);
                            Out::valore($this->nameForm . '_SETTORE', $Anauni_set['UNIDES']);
                            Out::valore($this->nameForm . '_PROPAS[PROSER]', '');
                            Out::valore($this->nameForm . '_SERVIZIO', '');
                        }
                        break;
                    case $this->nameForm . '_PROPAS[PROSER]':
                        $codice = $_POST[$this->nameForm . '_PROPAS']['PROSER'];
                        if (trim($codice) != "") {
                            $codice = str_repeat("0", 6 - strlen(trim($codice))) . trim($codice);
                            $Anauni_ser = $this->praLib->GetAnauniServ($_POST[$this->nameForm . '_PROPAS']['PROSET'], $codice);
                            if ($Anauni_ser) {
                                Out::valore($this->nameForm . '_PROPAS[PROSER]', $codice);
                                Out::valore($this->nameForm . '_SERVIZIO', $Anauni_ser['UNIDES']);
                            } else {
                                Out::valore($this->nameForm . '_PROPAS[PROSER]', '');
                                Out::valore($this->nameForm . '_SERVIZIO', '');
                                Out::setFocus($this->nameForm . '_PROPAS[PROSER]');
                                Out::msgInfo('ATTENZIONE', 'Codice Servizio non corretto per il Settore ' . $_POST[$this->nameForm . '_PROPAS']['PROSET']);
                            }
                        }
                        break;
...
//                }
            ...
            case "returnUniset":
                ...
                if ($Anauni_set) {
                    Out::valore($this->nameForm . '_PROPAS[PROSET]', $Anauni_set['UNISET']);
                    Out::valore($this->nameForm . '_SETTORE', $Anauni_set['UNIDES']);
                    Out::valore($this->nameForm . '_PROPAS[PROSER]', '');
                    Out::valore($this->nameForm . '_SERVIZIO', '');
                }
            ...
            case "returnUniser":
                ...
                if ($Anauni_ser) {
                    Out::valore($this->nameForm . '_PROPAS[PROSER]', $Anauni_ser['UNISER']);
                    Out::valore($this->nameForm . '_SERVIZIO', $Anauni_ser['UNIDES']);
                }
                break;
            case 'returnAcqrList':
            ...
            case 'returnPropasANTECEDENTE':
                ...
                break;
            case 'returnFromPraPasso':
                $proges_rec = $this->praLib->GetProges($_POST['rowid'], 'rowid');
                if (!$proges_rec) {
                    Out::msgInfo("Attenzione", "Record Fascicolo non trovato.");
                    break;
                }
                $this->CercaDocumentiDaPratica($proges_rec['GESNUM']);
                break;
//            case 'returnProges':
...
//                    $this->DecodStatoPassoCH($propas_rec['PROSTCH']);
            ...
            case "returnClassificazioneDocumenti":
                ...
                break;
            case 'returnInviaCart':
                include_once ITA_BASE_PATH . "/apps/Pratiche/praFrontOfficeCartWs.class.php";

                //TODO: Gestire l'invio al CART
                ...
                foreach ($selDestRows as $riga) {
                    $arrayEnti[] = $this->destinatari[$riga];
                }

                foreach ($this->passAlle as $allegato) {
                    if ($allegato['FILEPATH']) {
                        $allegato['CONTENTTYPE'] = itaMimeTypeUtils::estraiEstensione($allegato['FILEPATH'], true, true); // 'application/x-pkcs7-mime';
                        $allegato['CID'] = itaUUID::getV4(); // 'abb5fd19e3d60669517ec9f8337228816ba0248da13aedf' . $num . '@apache.org';
                        $arrayAllegati[] = $allegato;
                    }
                }

                $param = array(
                    'GESNUM' => $this->currGesnum,
                    'KEYPASSO' => $this->keyPasso,
                    'OPERATION' => $_POST['valori']['OPERATION'],
                    'TIPOSTIMOLO' => $_POST['valori']['TIPOSTIMOLO'],
                    'ENTI' => $arrayEnti,
                    'ALLEGATI' => $arrayAllegati,
                    'OGGETTO' => $_POST['valori']['OGGETTO'],
                    'MESSAGGIO' => $_POST['valori']['MESSAGGIO'],
                    'ATTESA_GG' => $_POST['valori']['ATTESA_GG'],
                    'INTEGRAZIONE' => $_POST['valori']['INTEGRAZIONE'], // Vale per la Notifica, se l'ente terzo può richiedere Integrazioni
                    'GGMAXINTEGRA' => $_POST['valori']['GGMAXINTEGRA'],
                    'MSGINTEGRA' => $_POST['valori']['MSGINTEGRA']
                );
...
//                        break;
                ...
                $praFOCart = new praFrontOfficeCartWs;
                $idCart = $praFOCart->invioCart($param);

                $this->Dettaglio($this->keyPasso);

                if ($_POST['valori']['GIORNIRISPOSTA'] > 0) {
                    Out::valore($this->nameForm . "_PRACOM[COMGRS]", $_POST['valori']['GIORNIRISPOSTA']);
                }

                //TODO: Aggiornare PROPAS
                //$this->AggiornaRecord();
                ...
                /*
                  ...
                  if ($destinatario['TIPOINVIO'] == 'CART'){
                  $arrayEnti[] = $destinatario;
                  }
                  }
                  ...
                  //                        out::msgInfo("Allegati", print_r($arrayAllegati, true));
                  //                        return;
                  ...
                  'TIPOSTIMOLO' => 'notifica',  // 'comunicazione',
                  'ENTI' => $arrayEnti,
                  'ALLEGATI' => $arrayAllegati,
                  'OGGETTO' => 'Comunicazione Generico',
                  'MESSAGGIO' => 'Corpo del messaggio',
                  'INTEGRAZIONE' => 'true'     // Vale per la Notifica, se l'ente terzo può richiedere Integrazioni
                  );
...
//$this->caricaTestoBase($_POST['retKey'], 'rowid');
                $this->passAlle = $this->praLib->caricaTestoBase($this->passAlle, $_POST['retKey'], 'rowid');
                $this->ContaSizeAllegati($this->passAlle);
                $this->CaricaGriglia($this->gridAllegati, $this->passAlle, '1', '100000');
                $this->bloccoAllegatiRiservati();
                $this->BloccaDescAllegati();
                        ...
                        switch ($_POST['colName']) {
                            case 'PREVIEW':
                                if (strtolower($ext) == "pdf") {
                                    ...
                                    if (file_exists($this->praLibAllegati->getFilenameFoglioElaborato($doc['FILEPATH'], true))) {
...
//                                }
                                ...
                                $this->CaricaGriglia($this->gridAllegati, $this->passAlle, "1", "500");
                                $this->BloccaDescAllegati();
                            ...
                            case "EDIT";
                                ...
                                if ($zipTree) {
                                    ...
                                    praRic::GetExplodedZip($this->nameForm, $arrayExplodeZip, $doc['FILEORIG']);
                                } else {
                                    $arrayExplodeZip = $this->praLib->explodeZipDirPlain($zipPath, $arrayZip, '', $doc['PROVENIENZA']);
                                    praRic::GetExplodedZip($this->nameForm, $arrayExplodeZip, $doc['FILEORIG'], false);
                                }
                                break;
...
//                                break;
                                if ($destinatario['IDMAIL']) {
                                    ...
                                    if ($destinatario['IDMESSAGGIOCART']) {

                                        out::msgInfo("Attenzione", "Visualizzazione Invio Cart in fase di sviluppo");
                                    }
                                }
                                break;
                            case 'CONSEGNA':
                                if ($destinatario['IDMAIL']) {
                                    ...
                                    if ($destinatario['IDMESSAGGIOCART']) {
                                        ...
                                        out::msgInfo("Attenzione", "Visualizzazione Consegna Cart in fase di sviluppo");
                                    }
                                }
                                break;
                            case 'ACCETTAZIONE':
...
//                                }
                                ...
                                if ($destinatario['IDMAIL']) {
                                    ...
                                    if ($destinatario['IDMESSAGGIOCART']) {
                                        // VEdere se messaggio inviato è stato annullato
                                        $annullato = false;
                                        $sql = "SELECT * FROM CART_INVIO WHERE CART_INVIO.IDMESSAGGIO = '" . $destinatario['IDMESSAGGIOCART'] . "'";
                                        $cart_invio_rec = ItaDB::DBSQLSelect($praLibCart->getITALWEB(), $sql, false);
                                        if ($cart_invio_rec) {
                                        ...
                                        if ($annullato) {
                                            out::msgInfo("Attenzione", "L'invio è già stato annullato.");
                                        } else {
                                            out::msgInfo("Attenzione", "L'invio non può essere annullato. E' possibile inviare un messaggio di Segnalazione errore ");
                                        }
                                    }
                                }
            ...
            case 'returnCampiPdf':
                ...
                $this->BloccaDescAllegati();
                break;
//            case 'returnCtrPassword':
//                if ($_POST['returnVal']) {
//                    $pracom_rec = $this->praLib->GetPracom($_POST['returnVal'], "rowid");
//                    $pracom_rec['COMDAT'] = "";
//                    $pracom_rec['COMORA'] = "";
//                    $update_Info = "Oggetto : Sblocco comunicazione in partenza n. " . $_POST['returnVal'];
//                    if (!$this->updateRecord($this->PRAM_DB, 'PRACOM', $pracom_rec, $update_Info)) {
//                        Out::msgStop("Errore", "Aggiornamento su PRACOM fallito");
//                        break;
//                    }
...
//                Out::valore($this->nameForm . '_MITTENTE', $anagra_rec['CODTRI']);
//                Out::valore($this->nameForm . '_DESC_MITTENTE', $nome . " " . $cognome);
//                Out::valore($this->nameForm . '_CODFISC_MITTENTE', $lavoro_rec['FISCAL']);
//                if ($lavoro_rec['FISCAL']) {
...
//Out::valore($this->nameForm . "_EMAIL_MITTENTE", $anamed_rec['MEDEMA']);
            ...
            case 'returnAnades':
                ...
                foreach ($arrSogg as $key => $rowid) {
                    ...
                    //TODO: Vedere come riempire il campo ['DESTCART'] 
                ...
                break;
            case "returnAnaddo":
                if ($_POST['retid'] == "Qualificati") {
                    //Out::msgInfo("Destinatari presenti", print_r($this->destinatari, true));
                    $destSel = explode(",", $_POST['retKey']);
                    foreach ($destSel as $dest) {
                        $anaddo_rec = $this->praLib->GetAnaddo($dest, "rowid");
                        if ($anaddo_rec) {
                            //Prima di caricare il vettore $sogg, assicurarsi che non sia già presnte in $this->destinatari[]
                            if ($this->ctrDestinazione($anaddo_rec['DDOCOD'], $anaddo_rec['DDONOM'])) {
                                $sogg = array();
                                $sogg['ROWID'] = 0;
                                $sogg['CODICE'] = $anaddo_rec['DDOCOD'];
                                ...
                                $sogg['DESTCART'] = $anaddo_rec['DDOCART'];
                                if ($sogg['DESTCART']) {
                                    $sogg['TIPOINVIO'] = "CART";
                                } else {
                                    $sogg['TIPOINVIO'] = '';
                    ...
                    //Out::msgInfo("Destinatari dopo", print_r($this->destinatari, true));


                    $this->CaricaGriglia($this->gridAltriDestinatari, $this->destinatari, "1", "100");

                    $this->attivaInvioCart();
                    // Out::msgInfo("Destinatari correnti", print_r($this->destinatari,true));
                } else {
            ...
            case 'returnDestinazioni':
                ...
                $this->caricaDestinazioniAllegato($this->passAlle[$_POST['rowidAlle']]['PASDEST']);
                $strDest = $this->praLibAllegati->getStringDestinatari($this->passAlle[$_POST['rowidAlle']]['PASDEST']);
...
//        $returnModel();
    ...
    public function Nascondi() {
        ...
        Out::hide($this->nameForm . '_InviaCart');
        ...
        Out::hide($this->nameForm . '_utenteCrea');
...
//}
        ...
        Out::hide($this->nameForm . '_PubblicaArticolo');
    }
...
//}
    ...
    public function Dettaglio($rowid, $tipo = 'propak') {
        ...
        $Serie_rec = $this->praLib->ElaboraProgesSerie($this->currGesnum);
        Out::valore($this->nameForm . '_Pratica', $Serie_rec);
        $this->keyPasso = $propas_rec['PROPAK'];
        $this->flagAssegnazioniPasso = $this->CheckAssegnazionePasso();
        $this->iniDateEdit = $propas_rec['PRODATEEDIT'];
        $this->noteManager = praNoteManager::getInstance($this->praLib, praNoteManager::NOTE_CLASS_PROPAS, $this->keyPasso);
        ...
        if ($this->tipoProtocollo == 'Iride' || $this->tipoProtocollo == 'Jiride' || $this->tipoProtocollo == 'Paleo4' || $this->tipoProtocollo == 'Italsoft-ws' || $this->tipoProtocollo == 'CiviliaNext' || $this->tipoProtocollo == 'Paleo41') {
...
//se c'è il numero di protocollo e non ci sono i metadati le icone non devono essere visibili
        ...
        if ($propas_rec['PROUTEADD'] == "@ADMIN@" && $propas_rec['PROUTEEDIT'] == "@ADMIN@") {
            ...
            if ($propas_rec['PROUTEADD'] != "") {
                Out::show($this->nameForm . "_utenteCrea");
                Out::valore($this->nameForm . "_utenteCrea", "Passo creato dall'utente " . $propas_rec['PROUTEADD'] . " in data " . $propas_rec['PRODATEADD']);
            }
        }
        if ($propas_rec['PROKPRE']) {
...
//            }
            if (($tipoProt == 'Jiride' || $tipoProt == 'Paleo4' || $tipoProt == 'Paleo41' || $tipoProt == 'Italsoft-ws') && $Metadati['DatiProtocollazione']['idMail']) {
...
//$Ananom_rec = $this->praLib->GetAnanom($this->profilo['COD_ANANOM']);
        ...
        if ($this->daMail['protocolla']['ANTECEDENTE']) {
            ...
            Out::valore($this->nameForm . "_SEQANTECEDENTE", $propas_rec_Antecedente['PROSEQ']);
            Out::valore($this->nameForm . "_DESCANTECEDENTE", $propas_rec_Antecedente['PRODPA']);
            Out::valore($this->nameForm . "_PROPAS[PRORPA]", $Ananom_rec_ant['NOMRES']);
            Out::valore($this->nameForm . "_RESPONSABILE", $Ananom_rec_ant['NOMCOG'] . " " . $Ananom_rec_ant['NOMNOM']);
        ...
        if ($this->tipoPasso == 2) {
            Out::valore($this->nameForm . "_PROPAS[PROSEQ]", $_POST[$this->nameForm . '_PROPAS']['PROSEQ'] + 1);
            Out::valore($this->nameForm . "_SEQANTECEDENTE", $_POST[$this->nameForm . '_PROPAS']['PROSEQ']);
            Out::valore($this->nameForm . "_DESCANTECEDENTE", $_POST[$this->nameForm . '_PROPAS']['PRODPA']);
...
//
        ...
        if ($allegati) {
            ...
            praRic::praElencoAllegatiProc($allegati, $this->nameForm, "", "Selezionare gli allegati desiderati per la pratica n. <b>$praticaFormattata</b>");
        } else {
...
//'TIPO' => 'Passo seq. ' . $propas_rec['PROSEQ'],
        ...
        if ($allegati) {
            $this->allegatiAppoggio = $allegati;
            if ($composer) {
                praRic::praComponiPDF($allegati, $this->nameForm);
            } else {
                praRic::praElencoAllegatiProc($allegati, $this->nameForm);
            }
        } else {
            Out::msgInfo("Ricerca Allegati", "Allegati non trovati");
        }
...
//                }
    ...
    function CaricaDestinatari($keyPasso, $sidx = 'SEQUENZA', $sord = 'ASC') {
        ...
        foreach ($this->destinatari as $key => $dest) {
            ...
            if ($dest['IDMAIL']) {
                ...
                if ($dest['IDMESSAGGIOCART']) {
                    $praLibCart = new praLibCart();
                    $icon = $praLibCart->GetIconCartInvio($dest['IDMESSAGGIOCART'], $keyPasso);
    ...
    function GetHtmlRiepilogoDest() {
        ...
        if ($this->destinatari) {
            ...
            foreach ($this->destinatari as $destinatario) {
                if ($destinatario['TIPOINVIO'] == 'CART') {
                    $html .= "<span style=\"font-size:1.2em;color:blue;\">" . $destinatario['NOME'] . ": Codice CART - " . $destinatario['DESTCART'] . "</span><br>";
                } else {
...
//Out::valore($this->nameForm . '_PROTRIC_DESTINATARIO', $pracomP_rec['COMPRT']);
    ...
    function GetDestinatariCartInvio() {
        $arrayDest = array();
        foreach ($this->destinatari as $keyDest => $dest) {
            if ($dest['TIPOINVIO'] != "CART") {
                continue;
            }
            if ($dest['DESTCART'] == "") {
                continue;
...
//                    return false;
//                }
                Out::valore($this->nameForm . '_SEQANTECEDENTE', $propas_rec['PROSEQ']);
                Out::valore($this->nameForm . '_DESCANTECEDENTE', $propas_rec['PRODPA']);
...
//
        ...
        $tableDef = ItaDB::DBGetTableObject($this->PRAM_DB, 'PROPAS');
        $propas_rec = $this->manageAudit(itaModelService::OPERATION_UPDATE, $tableDef, $propas_rec, App::$utente->getKey('nomeUtente'));
    ...
    private function caricaAllegatiEsterni($lista = "", $pasdoc_rec = array()) {
        ...
        if ($posEsterno == -1) {
            ...
            foreach ($lista as $key => $uplAllegato) {
                ...
                $funzAgg = $this->praLibAllegati->GetImgPreview($ext, pathinfo($uplAllegato['FILEPATH'], PATHINFO_DIRNAME), $uplAllegato);
                if ($this->flagAssegnazioniPasso) {
                    if (!$this->checkFrimaAllegato(false, $uplAllegato['ROWID'])) {
                        $funzAgg .= "<span style=\"display:inline-block;\" class=\"ita-icon ita-icon-sigillo-24x24\" title=\"Allegato messo allafirma\"></span>";
                    }
                }
            ...
            foreach ($lista as $key => $uplAllegato) {
                ...
                $funzAgg = $this->praLibAllegati->GetImgPreview($ext, pathinfo($uplAllegato['FILEPATH'], PATHINFO_DIRNAME), $uplAllegato);
                if ($this->flagAssegnazioniPasso) {
                    if (!$this->checkFrimaAllegato(false)) {
                        $funzAgg .= "<span style=\"display:inline-block;\" class=\"ita-icon ita-icon-sigillo-24x24\" title=\"Allegato messo allafirma\"></span>";
                    }
                }
        ...
        if (trim($email) != "") {
            ...
            if ($anades_rec) {
                ...
                if (count($anamed_tab) == 1) {
                    ...
                    proRic::proRicAnamed($this->nameForm, " WHERE MEDEMA='$email'", "", "assegnaAPasso", "returnanamed", "", $msgDetail);
                } else {
...
//
    ...
    private function SegnaturaAllegati($rowidScelti, $param) {
        foreach ($rowidScelti as $id) {
            ...
            /*
             * Aggiornamento SHA2 Allegato dopo marcatura
             */
            $newSha2 = hash_file('sha256', $pramPath . "/" . $pasdoc_rec['PASFIL']);
            $pasdoc_rec['PASSHA2'] = $newSha2;
            $update_Info = "Oggetto: Aggiornamento SHA2 dopo marcatura allegato " . $pasdoc_rec['PASNAME'];
            if (!$this->updateRecord($this->PRAM_DB, 'PASDOC', $pasdoc_rec, $update_Info)) {
                Out::msgStop("ERRORE", "Errore Aggiornamento SHA2 dopo marcatura allegato " . $pasdoc_rec['PASNAME']);
                return false;
            }
        }
        return true;
...
//        $PARMENTE_rec = $this->utiEnte->GetParametriEnte();
        switch ($this->tipoProtocollo) {
            ...
            default:
                ...
                /*
                 * Alla fine di tutto vado al dettaglio e ricarico il passo
                 */
                $this->Dettaglio($this->keyPasso, "propak");
                break;
        }
...
//Implementiamo i controlli
    ...
    public function SincronizzaRecordPasso($Propas) {
        ...
        $tableDef = ItaDB::DBGetTableObject($this->PRAM_DB, 'PROPAS');
        $Propas = $this->manageAudit(itaModelService::OPERATION_UPDATE, $tableDef, $Propas, App::$utente->getKey('nomeUtente'));
...
//$keyRic = $proges_rec['GESPRO'] . substr($propas_rec['PROPAK'], 10);
        ...
        if ($keyRic) {
            ...
            $ret = praRic::praItepasAntecedenti($this->nameForm, $itepas_rec, $this->PRAM_DB);
            if (!$ret) {
                $this->AzzeraVariabili();
                $this->apriInserimento($this->currGesnum);
            } else {
    ...
    function caricaHeader() {
        ...
        $praSoggetti = praSoggetti::getInstance($this->praLib, $this->currGesnum, $extraParam);
        $soggettiList = "";
        foreach ($praSoggetti->soggetti as $soggetto) {

            if (trim($soggetto['DESNOM'])) {
                $desnom = $soggetto['DESNOM'];
            } else {
                if (trim($soggetto['DESCOGNOME'])) {
                    $desnom = $soggetto['DESCOGNOME'] . " " . $soggetto['DESNOME'];
                } else {
                    if (trim($soggetto['DESRAGSOC'])) {
                        $desnom = $soggetto['DESRAGSOC'];
                    }
                }
            }
            $anaruo_rec = $this->praLib->GetAnaruo($soggetto['DESRUO']);
            $soggettiList .= "<div><span style=\"display:inline-block;width:150px;\"><b>{$anaruo_rec['RUODES']}</b></span><span style=\"display:inline-block;color:black;width:200px\">$desnom</span><span style=\"display:inline-block;color:black;width:150px\"><b>Cel. </b>{$soggetto['DESCEL']}</span><span style=\"display:inline-block;color:black;width:150px\"><b>Tel. </b>{$soggetto['DESTEL']} </span><span><b>Mail </b>{$soggetto['DESEMA']}</span></div>";
            ;
        }
        $gesnpr = "";
        if ($proges_rec['GESNPR'] != 0) {
            $gesnpr = "<div><span style=\"display:inline-block;width:150px;\"><b>Protocollo:</b></span><span style=\"color:DodgerBlue;\"><b>" . intval(substr($proges_rec['GESNPR'], 4, 6)) . "/" . substr($proges_rec['GESNPR'], 0, 4) . "</b></span></div>";
        }
        $codproc = "";
        if ($proges_rec['GESCODPROC']) {
            $codproc = "<div><span style=\"display:inline-block;width:150px;\"><b>Codice Procedura:</b></span><span style=\"color:DarkRed;\"><b>" . $proges_rec['GESCODPROC'] . "</b></span></div>";
        }


        $htmlHeader = "<div style=\"width:1200px;\">";
        if ($anaset_rec['SETDES'] || $anaatt_rec['ATTDES']) {
            $htmlHeader .= "<div>" . $anaset_rec['SETDES'] . " - " . $anaatt_rec['ATTDES'] . "</div>";
        }
        $htmlHeader .= "<div>" . $anapra_rec['PRADES__1'] . $anapra_rec['PRADES__2'] . $anapra_rec['PRADES__3'] . "</div>";
        if ($proges_rec['GESOGG']) {
            $htmlHeader .= "<div>" . $proges_rec['GESOGG'] . "</div>";
        }
        if ($gesnpr) {
            $htmlHeader .= $gesnpr;
        }
        ...
        if ($soggettiList) {
            $htmlHeader .= $soggettiList;
        }
        $htmlHeader .= "</div>";

        Out::css($this->nameForm . "_Pratica_field", "vertical-align", "top");
        $Serie_rec = $this->praLib->ElaboraProgesSerie($this->currGesnum);
        Out::valore($this->nameForm . '_Pratica', $Serie_rec);
        Out::html($this->nameForm . '_divDettProcedimento', $htmlHeader);
        /*
         * 
         */
    }
...
//$Allegati = $this->datiWS['RetValue']['Dati']['Allegati']['Allegato'];
    ...
    public function getArrayDataTreeAllegato($dataDetail_rec, $duplica, $inc, $flagCdsPasso, $flagDwOnlinePasso) {
        ...
        if ($this->flagAssegnazioniPasso) {
            if (!$this->checkFrimaAllegato(false, $dataDetail_rec['ROWID'])) {
...
//        $arrayDataTmp['isLeaf'] = 'true';
    ...
    public function caricaTestoClass($Rowid = '') {
        ...
        if ($DocClass['CODICEDOC']) {
            ...
            $this->passAlle = $this->praLib->caricaTestoBase($this->passAlle, $Documento['ROWID'], 'rowid');
            $this->ContaSizeAllegati($this->passAlle);
            $this->CaricaGriglia($this->gridAllegati, $this->passAlle, '1', '100000');
            $this->bloccoAllegatiRiservati();
            $this->BloccaDescAllegati();
    ...
    public function caricaDestinatarioFromAnamed($codice, $tipo = "codice") {
        ...
        $anaddo_rec = $this->praLib->GetAnaddo($anamed_rec['MEDCOD']);
        if ($anaddo_rec) {
            $sogg['DESTCART'] = $anaddo_rec['DDOCART'];
            if ($sogg['DESTCART']) {
                $sogg['TIPOINVIO'] = "CART";
    ...
    function ctrDestinazione($codNuovo, $descNuovo) {
        foreach ($this->destinatari as $dest) {
            if ($dest['CODICE'] == $codNuovo) {
                return false;
            }
        }
        return true;
    ...
    private function cancellaDestinatarioAllegatoPasso($codDestinatario) {
        $trovato = false;
        //TODO: Pulire anche il vettore $this->passAlle indipendentemente da PASDOC, per i casi in cui non siano ancora salvati
        //
        // Si scorrono gli allegati associati al passo e se nel campo PASDOC.PASDES c'è il destinatario lo cancelliamo
        $allegati_tab = $this->praLib->GetPasdoc($this->keyPasso, 'codice', true);

        if ($allegati_tab) {

            foreach ($allegati_tab as $allegato_rec) {
                $arrayDest = unserialize($allegato_rec['PASDEST']);
                if (is_array($arrayDest)) {
                    //                Out::msginfo("destinazione allegati", print_r($arrayDest, true));

                    if (in_array($codDestinatario, $arrayDest)) {
                        //Out::msginfo("Origine destinatari ", print_r($arrayDest, true));
                        //Out::msginfo("RowId PASDOC ", $allegato_rec['ROWID']);
                        // rimuovo il valore passando ad unset la chiave dell'item
                        // recuperata usando array_search
                        unset($arrayDest[array_search($codDestinatario, $arrayDest)]);

                        //Out::msginfo("array passAlle ", print_r($this->passAlle, true));


                        $allegato_rec['PASDEST'] = serialize($arrayDest);

                        // Salvare record PASDOC
                        $update_info = 'Oggetto: Tolto destinatario' . $codDestinatario . ' dal record PASDOC CON ROWID =' . $allegato_rec['FOTIPO'];

                        if ($this->updateRecord($this->PRAM_DB, 'PASDOC', $allegato_rec, $update_info, 'ROWID')) {
                            $trovato = true;
                        }

                        //                    $passAlle_tmp = $this->praLib->cleanArrayTree($this->passAlle);
                        //                    $passAlle = array_values($passAlle_tmp);
                        //                    $idx = array_search($allegato_rec['ROWID'], array_column($passAlle, "ROWID"));
                        //
                        //                    $strDest = $this->praLibAllegati->getStringDestinatari($allegato_rec['PASDEST']);
                        //                    $this->passAlle[$idx]['DESTINAZIONI'] = $strDest;
                        //                    $this->passAlle[$idx]['PASDEST'] = $allegato_rec['PASDEST'];
                        //                    $this->CaricaGriglia($this->gridAllegati, $this->passAlle);
                    }
                }
            }
        ...
        // Sistemo array $this->passAlle
        foreach ($this->passAlle as $key => $elemento) {

            $arrayDest = unserialize($elemento['PASDEST']);
            if (is_array($arrayDest)) {
//                Out::msginfo("destinazione allegati", print_r($arrayDest, true));
                ...
                if (in_array($codDestinatario, $arrayDest)) {
                    ...
                    $this->passAlle[$key]['PASDEST'] = serialize($arrayDest);

                    $strDest = $this->praLibAllegati->getStringDestinatari($this->passAlle[$key]['PASDEST']);
                    $this->passAlle[$key]['DESTINAZIONI'] = "<div class=\"ita-html\"><span style=\"color:red;\" class=\"ita-Wordwrap ita-tooltip\" title=\"$strDest\"><b>" . count($arrayDest) . "</b></span></div>";

                    //Out::msginfo("Elenco enti ", print_r($strDest, true));
        ...
        //Out::msginfo("codice ", print_r($codDestinatario, true));

        if ($trovato) {
            // Out::msginfo("passAlle finale", print_r($this->passAlle, true));
    ...
    private function attivaInvioCart() {
        $cart = false;
        Out::hide($this->nameForm . '_InviaCart');
        Out::hide($this->nameForm . '_DestinatarioCart');

        $codice = array();
        $codice['GESNUM'] = $this->currGesnum;
        $codice['PROPAK'] = '';
        ...
        if ($praFoList_rec) {
            ...
            if ($praFoList_rec['FOTIPO'] == praFrontOfficeManager::TYPE_FO_CART_WS) {   // 'cart-ws'
                $cart = true;
            }
        }

        if ($cart) {
            Out::show($this->nameForm . '_DestinatarioCart');
            $cartDest = false;
            foreach ($this->destinatari as $destinatario) {
                if ($destinatario['TIPOINVIO'] == 'CART') {
                    $cartDest = true;
            ...
            if ($cartDest) {
                Out::show($this->nameForm . '_InviaCart');
            }
        }
    }

    // Gestione Pubblicazine Articolo su FO Remoto
    private function gestPubbArticoloFORemoto($tipoOperazione = 'Insert') {
        ...
        $sistemaArticolo = false;
        if ($tipoOperazione == 'Delete') {
            // Se cancello il Passo, devo sistemare il FO solo se valorizzata la data PRODATEPUBART
            // Passo inviato al FO, perchè campo PRODATEPUBART valorizzato
            if ($propas_rec['PRODATEPUBART'] && $propas_rec['PRODATEPUBART'] != '0000-00-00') {
                $sistemaArticolo = true;
            }
        } else {
            // Check su "Pubblica Articolo"
            if ($propas_rec['PROPART'] == 1) {
                $sistemaArticolo = true;
            } else {
                // Passo inviato al FO, perchè campo PRODATEPUBART valorizzato
        ...
        $ret_esito = praFrontOfficeManager::caricaArticoliFO("italsoft-ws", $this->currGesnum, $this->keyPasso, $istanza, $tipoOperazione, $pubArticolo, $pubAllegati);
        if ($ret_esito === false) {
            Out::msgStop("Errore nella pubblicazione articolo", praFrontOfficeManager::$lasErrMessage);
    ...
    private function abilitaPubblicaArticolo() {
        Out::hide($this->nameForm . '_PubblicaArticolo');
        Out::hide($this->nameForm . "_dataPubArticolo");
        ...
        $this->pubblicaArticolo($propas_rec['PROPART'], $propas_rec['PRODATEPUBART'], $propas_rec['PROTIMEPUBART']);

//        Out::msgInfo("POST", print_r($_POST[$this->nameForm. "_PROPAS"], true));
//        Out::msgInfo("Pub. Art", print_r($_POST[$this->nameForm. "_PROPAS"]['PROPART'], true));
    }

    private function pubblicaArticolo($pubblica, $dataPubblicazione, $oraPubblicazione) {
        Out::hide($this->nameForm . '_PubblicaArticolo');
        Out::hide($this->nameForm . "_dataPubArticolo");
        ...
        Out::show($this->nameForm . '_PubblicaArticolo');
        if ($dataPubblicazione) {
            Out::show($this->nameForm . "_dataPubArticolo");
            $data1 = new DateTime($dataPubblicazione);
            $dataVisualizzazione = $data1->format('d/m/Y');
            Out::valore($this->nameForm . "_dataPubArticolo", "Articolo Pubblicato il " . $dataVisualizzazione . " " . $oraPubblicazione);
        }
    }
     ...
     */
    public function manageAudit($operationType, $tableDef, $data, $utente) {
