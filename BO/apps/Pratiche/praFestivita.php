...
function praFestivita() {
    $praFestivita = new praFestivita();
    $praFestivita->parseEvent();
    return;
}

class praFestivita extends itaModel {

    public $PRAM_DB;
    public $praLib;
    public $nameForm = "praFestivita";
    public $divRis = "praFestivita_divRisultato";
    public $gridGiorniFestivi = "praFestivita_gridGiorniFestivi";
    ...
    public function parseEvent() {
        ...
        switch ($_POST['event']) {
                ...
                switch ($_POST['id']) {
                    case $this->gridGiorniFestivi:
                        ...
                        if ($_POST['cellname'] == 'NOMEFESTA') {
                            $sql = "SELECT * FROM TABFESTA WHERE NOMEFESTA= ' " . $_POST['value'] . " ' ";
                            $GiorniFestivi_rec = ItaDB::DBSQLSelect($this->PRAM_DB, $sql);
                        ...
                        $this->updateRecord($this->PRAM_DB, 'TABFESTA', $GiorniFestivi_rec, '', 'ROWID');
                        $this->OpenRisultato();
                        break;
                }
                break;
            ...
            case 'exportTableToExcel':
                $sql = $this->CreaSql();
                $ita_grid01 = new TableView($this->gridGiorniFestivi, array('sqlDB' => $this->PRAM_DB, 'sqlQuery' => $sql));
            ...
            case 'onClick':
                switch ($_POST['id']) {
                    ...
                    case $this->nameForm . '_ConfermaCancella':
                        ...
                        $GiorniFestivi_rec = $_POST[$this->nameForm . '_TABFESTA'];
                        $delete_Info = 'Oggetto: ' . $GiorniFestivi_rec['NOMEFESTA'] . " " . $GiorniFestivi_rec['DATAFESTA'] . " ";
                        if ($this->deleteRecord($this->PRAM_DB, 'TABFESTA', $rowid, $delete_Info)) {
                            $this->openRisultato();
                        } else {
                    ...
                    case $this->nameForm . '_ConfermaRicalcolaAnno':
                        $sql = "SELECT * FROM TABFESTA ";
                        $GiorniFestivi_tab = ItaDB::DBSQLSelect($this->PRAM_DB, $sql, true);
                        foreach ($GiorniFestivi_tab as $GiorniFestivi_rec) {
                            ...
                            if (!$this->updateRecord($this->PRAM_DB, 'TABFESTA', $GiorniFestivi_rec, '')) {
    ...
    function Nuovo() {
        $sql = "SELECT * FROM TABFESTA WHERE DATAFESTA= '' ";
        $GiorniFestivi_rec = ItaDB::DBSQLSelect($this->PRAM_DB, $sql);
        ...
        if ($this->insertRecord($this->PRAM_DB, 'TABFESTA', $GiorniFestivi_rec, $insert_Info)) {
            $this->OpenRisultato();
        } else {
