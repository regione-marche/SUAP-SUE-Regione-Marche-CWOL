...
function praUniope() {
    $praUniope = new praUniope();
    $praUniope->parseEvent();
    return;
}

class praUniope extends itaModel {
    public $PRAM_DB;
    public $praLib;
    public $utiEnte;
    public $ITALWEB_DB;
    public $workDate;
    public $nameForm="praUniope";
    public $divGes="praUniope_divGestione";
    public $divRis="praUniope_divRisultato";
    public $divSto="praUniope_divStorico";
    public $divRic="praUniope_divRicerca";
    public $gridUniope="praUniope_gridUniope";
    public $gridStorico="praUniope_gridStorico";
    ...
    public function parseEvent() {
        ...
        switch ($_POST['event']) {
            ...
            case 'exportTableToExcel':
                $sql=$this->CreaSql();
                $ita_grid01 = new TableView($this->gridUniope,
                        array(
                                'sqlDB'=>$this->PRAM_DB,
                                'sqlQuery'=>$sql));
                $ita_grid01->setSortIndex('UNIDES');
                $ita_grid01->exportXLS('','Unità.xls');
                break;
            case 'onClickTablePager':
                $ordinamento=$_POST['sidx'];
                if ($ordinamento=='DESSET') {
                    $ordinamento='SETTORI.UNIDES';
                }
                if ($ordinamento=='DESSER') {
                    $ordinamento='SERVIZI.UNIDES';
                }
            ...
            case 'printTableToHTML':
                ...
                $itaJR->runSQLReportPDF($this->PRAM_DB,'praUniope', $parameters);
                break;
            case 'onClick': // Evento Onclick
                switch ($_POST['id']) {
                    ...
                    // Importo l'ordinamento del filtro
                        ...
                        try {   // Effettuo la FIND
                            ...
                            if (!$ita_grid01->getDataPageFromArray('json',$Result_tab)) {
                                ...
                                TableView::enableEvents($this->gridUniope);
                    ...
                    case $this->nameForm.'_Aggiungi':
                        $codice=$_POST[$this->nameForm.'_ANAUNI']['UNIOPE'];
                        ...
                        $_POST[$this->nameForm.'_ANAUNI']['UNIOPE']=$codice;
                        $Anauni_ric=$this->praLib->GetAnauniOpe($settore, $servizio, $codice);
                        if (!$Anauni_ric) {
                            $Anauni_rec=$_POST[$this->nameForm.'_ANAUNI'];
                            try {
                                $insert_Info = 'Oggetto: ' . $Anauni_rec['UNIOPE'] . " " . $Anauni_rec['UNIDES'];
                    ...
                    case $this->nameForm.'_Aggiorna':
                        ...
                        if ($data!="") {
                            ...
                            Out::msgQuestion("Chiusura Unità Operativa", 'Confermando '.$Ananom_rec["NOMCOG"].' '.$Ananom_rec["NOMNOM"].' non sarà più responsabile dell unità operativa <br>'.$Anauni_rec["UNIDES"].' in data '.$data,
                                    array(
                            ...
                            $codice=$Anauni_rec['UNIOPE'];
                            $codice=str_repeat("0",6-strlen(trim($codice))).trim($codice);
                            $Anauni_rec['UNIOPE']=$codice;
                    ...
                    case $this->nameForm.'_ConfermaCancella':
                        $Anauni_rec=$_POST[$this->nameForm.'_ANAUNI'];
                        try {
                            $delete_Info = 'Oggetto: ' . $Anauni_rec['UNIOPE'] . " " . $Anauni_rec['UNIDES'];
                            if ($this->deleteRecord($this->PRAM_DB, 'ANAUNI', $Anauni_rec['ROWID'], $delete_Info)) {
                                $this->OpenRicerca();
                            }

                        }catch (Exception $e) {
                            Out::msgStop("Errore in Cancellazione su ANAGRAFICA UNITA OPERATIVE",$e->getMessage());
                        }
                        break;
                    ...
                    case $this->nameForm.'_ANAUNI[UNIRES]_butt':
                        praRic::praRicAnanom($this->PRAM_DB, "praUniope", "RICERCA DIPENDENTI", "", "returnUnires" );
                        break;

                    case $this->nameForm.'_ANAUNI[UNISET]_butt':
                    case $this->nameForm.'_Uniset_butt':
                        praRic::praRicAnauni("praUniope", "RICERCA SETTORI", "returnUniset" );
                        break;
                    ...
                    case $this->nameForm.'_Uniser_butt':
                        ...
                        praRic::praRicAnaSer($this->PRAM_DB, "praUniope", "RICERCA SERVIZI", $where, "returnUniser" );
                        break;
                    case 'close-portlet':
                        $this->returnToParent();
                        break;
            ...
            case 'onBlur':
                switch ($_POST['id']) {
                    ...
                    case $this->nameForm.'_Uniope':
                        ...
                        $servizio=$_POST[$this->nameForm.'_Uniser'];
                        $codice=$_POST[$this->nameForm.'_Uniope'];
                        if (trim($codice)!="") {
                            $codice=str_repeat("0",6-strlen(trim($codice))).trim($codice);
                            if (trim($servizio)!="" AND trim($settore)!="" ) {
                                $sql="SELECT * FROM ANAUNI WHERE UNISET='".$settore."' AND UNISER='".$servizio."' AND UNIOPE='".$codice."' AND UNIAPE=''";
                                ...
                                Out::valore($this->nameForm.'_Uniope',$codice);
                            }
                        }
                        break;
                    case $this->nameForm.'_ANAUNI[UNIOPE]':
                        ...
                        if (trim($codice)!="") {
                            $codice=str_repeat("0",6-strlen(trim($codice))).trim($codice);
                            Out::valore($this->nameForm.'_ANAUNI[UNIOPE]',$codice);
                        }
                        break;
...
//                    Out::codice('closeCurrDialog();');
    ...
    function CreaSql() {
        ...
        $sql="";
        // Importo l'ordinamento del filtro
        $sql="SELECT ANAUNI.ROWID AS ROWID, ANAUNI.UNIOPE AS UNIOPE, ANAUNI.UNISET AS UNISET, ANAUNI.UNIRES AS UNIRES, ANAUNI.UNIDES AS UNIDES,
                ANAUNI.UNISER AS UNISER, SETTORI.UNIDES AS DESSET, SERVIZI.UNIDES AS DESSER, NOMCOG & ' ' & NOMNOM AS NOMCOG
...
//        $sql="SELECT ANAUNI.ROWID AS ROWID, UNISET, UNIDES, UNISER, UNIOPE, ".
//                $this->PRAM_DB->strConcat("NOMCOG", "' '", "NOMNOM")." AS NOMCOG, ".
//                $this->PRAM_DB->strConcat("UNISET", "' - '", "UNISER", "' - '", "UNIOPE")." AS SETSEROPE
...
//            $sql .= " AND UNISET = '".$_POST[$this->nameForm.'_Uniset']."' AND UNISER<>'' AND UNIOPE<>'' AND UNIAPE=''";
        }
...
//            $sql .= " AND UNISER = '".$_POST[$this->nameForm.'_Uniser']."' AND UNISET = '".$_POST[$this->nameForm.'_Uniset']."' AND UNIAPE=''";
        }
        if ($_POST[$this->nameForm.'_Uniope']!="") {
            $sql .= " AND ANAUNI.UNIOPE = '".$_POST[$this->nameForm.'_Uniope']."'";
//            $sql .= " AND UNIOPE = '".$_POST[$this->nameForm.'_Uniope']."' AND UNISER<>'' AND UNISET<>'' AND UNIAPE=''";
        }

        if ($_POST[$this->nameForm.'_Unides']!="") {
            $sql .= " AND ANAUNI.UNIDES LIKE '%".addslashes($_POST[$this->nameForm.'_Unides'])."%'";
//            $sql .= " AND UNIDES LIKE '%".addslashes($_POST[$this->nameForm.'_Unides'])."%' AND UNISER<>'' AND UNISET<>'' AND UNIOPE<>'' AND UNIAPE=''";
        }
        return $sql;
    }
    ...
    function CreaSqlSto() {
        $sql="SELECT ANAUNI.ROWID AS ROWID, UNISET, UNIDES, UNIDAP, UNIAPE,
            ".$this->PRAM_DB->strConcat('NOMCOG', $this->PRAM_DB->strConcat("' '", 'NOMNOM'))." AS NOMCOG
            FROM ANAUNI ANAUNI LEFT OUTER JOIN ANANOM ANANOM
            ON ANAUNI.UNIRES=ANANOM.NOMRES
    ...
    function OpenRicerca() {
        ...
        Out::clearFields($this->nameForm, $this->divRic);
        Out::clearFields($this->nameForm, $this->divGes);
        TableView::disableEvents($this->gridUniope);
        TableView::clearGrid($this->gridUniope);
        $this->Nascondi();
        Out::show($this->nameForm.'_Nuovo');
    ...
    public function Dettaglio($_Indice) {
        ...
        $open_Info='Oggetto: ' . $Anauni_rec['UNIOPE'] . " " . $Anauni_rec['UNIDES'];
