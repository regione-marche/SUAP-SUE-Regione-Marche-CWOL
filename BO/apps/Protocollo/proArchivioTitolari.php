...
class proArchivioTitolari extends itaModel {
    ...
    public $Where = array();
    public $UltimoElementoSelezionato = array();
    ...
    function __construct() {
        ...
        $this->UltimoElementoSelezionato = App::$utente->getKey($this->nameForm . '_UltimoElementoSelezionato');
    }
    ...
    function __destruct() {
        ...
        if ($this->close != true) {
            ...
            App::$utente->setKey($this->nameForm . '_UltimoElementoSelezionato', $this->UltimoElementoSelezionato);
        }
    }
    ...
    public function parseEvent() {
        ...
        switch ($_POST['event']) {
            case 'openform':
                $this->OpenRicerca();
                if (!$this->proLibTitolario->CheckCreaVersioneZero()) {
                    Out::msgStop("Attenzione.", "Errore nel controllo versione.<br>" . $this->proLibTitolario->getErrMessage());
                }
                break;
            ...
            case 'dbClickRow':
                switch ($_POST['id']) {
                    case $this->gridVersioni:
                        // Test Titolario.
                        $rowid = $_POST['rowid'];
                        $Versione_rec = $this->proLibTitolario->GetVersione($rowid, 'rowid');
                        if (!$Versione_rec) {
                            Out::msgStop("Attenzione", "Versione non trovata.");
                            break;
                        }
                        $this->GestioneVersione($Versione_rec['VERSIONE_T']);
                        break;
                    case $this->gridTitolario:
                        // Test Titolario.
                        $rowid = $_POST['rowid'];
                        $Chiave = $this->Titolario[$rowid]['VALORE_CHIAVE'];
                        $TipoChiave = $this->Titolario[$rowid]['TIPO_CHIAVE'];
                        if (!$this->proLibTitolario->DettaglioElementoTitolario($this->nameForm, $Chiave, $TipoChiave)) {
                    ...
                    case $this->gridVersioni:
                        switch ($_POST['colName']) {
                            case 'DETTAGLIO_T':
                                ...
                                $this->Versione = $Versione_rec['VERSIONE_T'];
            ...
            case 'onClick':
                switch ($_POST['id']) {
                    ...
                    case $this->nameForm . '_NuovoTitolario':
                        $rowid = $_POST[$this->gridTitolario]['gridParam']['selarrrow'];
                        if (!$rowid || $rowid == null) {
                            Out::msgInfo("Attenzione", "Selezionare un elemento del titolario.");
                            break;
                        }

                        $Chiave = $this->Titolario[$rowid]['VALORE_CHIAVE'];
                        $TipoChiave = $this->Titolario[$rowid]['TIPO_CHIAVE'];
                        if (!$this->proLibTitolario->NuovoElementoTitolario($this->nameForm, $Chiave, $TipoChiave)) {
                            Out::msgStop("Attenzione", $this->proLibTitolario->getErrMessage());
                        }
                        break;
                    ...
                    case $this->nameForm . '_StampaTitolario':
                        $Anaent_rec = $this->proLib->GetAnaent('2');
                        include_once(ITA_LIB_PATH . '/itaPHPCore/itaJasperReport.class.php');
                        $itaJR = new itaJasperReport();
                        $sql = "SELECT * FROM AACVERS WHERE VERSIONE_T = ".$this->Versione ;
                        $parameters = array("Sql" => $sql, "Ente" => $Anaent_rec['ENTDE1']);
                        $itaJR->runSQLReportPDF($this->PROT_DB, 'proTitolario', $parameters);
                        break;
            ...
            case 'returnAggiuntaTitolario':
                $rowid = $this->formData[$this->gridTitolario]['gridParam']['selarrrow'];
                $this->CaricaTitolario();
                $rowSelect = $this->ForzaEspandiElementi($this->Titolario[$rowid]);
                $this->CaricaGrigliaTitolario();
                TableView::setSelection($this->gridTitolario, $rowSelect);
                break;
        }
    }
    ...
    public function close() {
        ...
        App::$utente->removeKey($this->nameForm . '_UltimoElementoSelezionato');
    ...
    public function Nascondi() {
        ...
        Out::hide($this->nameForm . '_StampaTitolario');
    }
    ...
    public function ElencaTitolario() {
        ...
        Out::show($this->nameForm . '_StampaTitolario');
    ...
    public function CaricaGrigliaTitolario() {
        ...
        $gridScheda->setPageNum(1);
        $gridScheda->setPageRows('10000000');
    ...
    public function CopiaVersioniTitolario($VersioneDestino = '') {
        ...
        $sql = "SELECT * FROM ANACAT WHERE VERSIONE_T = $VersioneDestino ";
        ...
        $sql = "SELECT * FROM ANACAT WHERE VERSIONE_T = {$this->Versione} AND CATDAT = '' ";
    ...
    public function ForzaEspandiElementi($ElementoPadre) {
        $rowSelected = '1';
        if ($ElementoPadre['CATCOD']) {
            switch ($ElementoPadre['TIPO_CHIAVE']) {
                case 'ANACAT':
                    foreach ($this->Titolario as $k => $Titolario) {
                        if ($Titolario['CATCOD'] == $ElementoPadre['CATCOD'] && $Titolario['CLACOD'] == '') {
                            $this->Titolario[$k]['expanded'] = 'true';
                        }
                    }
                    break;
                case 'ANACLA':
                    foreach ($this->Titolario as $k => $Titolario) {
                        if (($Titolario['CATCOD'] == $ElementoPadre['CATCOD'] && $Titolario['CLACOD'] == '') || ($Titolario['CATCOD'] == $ElementoPadre['CATCOD'] && $Titolario['CLACOD'] == $ElementoPadre['CLACOD'] && $Titolario['FASCOD'] == '')) {
                            $this->Titolario[$k]['expanded'] = 'true';
                        }
                    }
                    break;
            ...
            /* Cerco la riga selezionata corrispondente del nuovo titolario caricato. */
            foreach ($this->Titolario as $k => $Titolario) {
                if ($Titolario['CATCOD'] == $ElementoPadre['CATCOD'] && $Titolario['CLACOD'] == $ElementoPadre['CLACOD'] && $Titolario['FASCOD'] == $ElementoPadre['FASCOD']) {
                    $rowSelected = $k;
                }
            }
        }
