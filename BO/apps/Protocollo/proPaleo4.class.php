...
class proPaleo4 extends proWsClient {
...
//        );
        ...
        if (!$ret) {
            if ($paleoClient->getFault()) {
                $ritorno["Status"] = "-1";
                $ritorno["Message"] = $paleoClient->getFault();
                $ritorno["RetValue"] = false;
                return $ritorno;
            } elseif ($paleoClient->getError()) {
                $ritorno["Status"] = "-1";
                $ritorno["Message"] = $paleoClient->getError();
                $ritorno["RetValue"] = false;
                return $ritorno;
            }
            return;
...
//        return $ritorno;
        ...
        if ($TipoRisultato == "Error") {
            ...
            $ritorno["RetValue"] = array(
                'DatiProtocollazione' => array(
                    'TipoProtocollo' => array('value' => 'Paleo4', 'status' => true, 'msg' => 'ArchiviaDocumentoInterno'),
                    'DocNumber' => array('value' => $DocNumber, 'status' => true, 'msg' => ''),
                    'Segnatura' => array('value' => $Segnatura, 'status' => true, 'msg' => ''),
                    'Anno' => array('value' => $Anno, 'status' => true, 'msg' => ''),
...
//        }
        ...
        if ($TipoRisultato == "Error") {
            ...
            $ritorno["RetValue"] = array(
                'DatiProtocollazione' => array(
                    'TipoProtocollo' => array('value' => 'Paleo4', 'status' => true, 'msg' => 'ProtocollazionePartenza'),
                    'proNum' => array('value' => $proNum, 'status' => true, 'msg' => ''),
                    'Data' => array('value' => $Data, 'status' => true, 'msg' => ''),
                    'DocNumber' => array('value' => $DocNumber, 'status' => true, 'msg' => ''),
                    'Segnatura' => array('value' => $Segnatura, 'status' => true, 'msg' => ''),
    ...
    function LeggiProtocollo($param) {
        $ritorno = array();
        $ritorno["Status"] = "0";
        $ritorno["Message"] = "Lettura protocollo avvenuta con successo!";
        $ritorno["RetValue"] = true;
        ...
        if (!$ret) {
            if ($paleoClient->getFault()) {
                $msg = $paleoClient->getFault();
                $ritorno["Status"] = "-1";
                $ritorno["Message"] = "Rilevato un errore in fase di lettura protocollo: " . $msg . "";
                $ritorno["RetValue"] = false;
                return $ritorno;
            } elseif ($paleoClient->getError()) {
                $msg = $paleoClient->getError();
        ...
        if ($TipoRisultato == "Error") {
            ...
            $datiProtocollazione = array(
                'TipoProtocollo' => array('value' => 'Paleo4', 'status' => true, 'msg' => $msg),
    ...
    public function getClientType() {
        return proWsClientHelper::CLIENT_PALEO4;
    }
