...
class proGestVersioneTit extends itaModel {
    ...
    public function Nuovo() {
        ...
        Out::setFocus('', $this->nameForm . '_AAC_VERS[DESCRI]');
    }
    ...
    public function Dettaglio($Versione_T = '') {
        ...
        // Prendo i valori
        $sql = "SELECT * FROM AAC_VERS WHERE VERSIONE_T = $Versione_T ";
        ...
        Out::valori($Aac_vers_rec, $this->nameForm . '_AAC_VERS');
        Out::setFocus('', $this->nameForm . '_AAC_VERS[DESCRI]');
    }
    ...
    public function Aggiungi() {
        ...
        $Aac_Vers_rec = $_POST[$this->nameForm . '_AAC_VERS'];

        try {
            ItaDB::DBInsert($this->PROT_DB, 'AAC_VERS', 'VERSIONE_T', $Aac_Vers_rec);
        } catch (Exception $e) {
            $this->setErrCode(-1);
            $this->setErrMessage("Errore in inserimento Versione.<br> " . $e->getMessage());
            return false;
        }
    ...
    public function Aggiorna() {
        ...
        try {
            $nrow = ItaDB::DBUpdate($this->PROT_DB, 'AAC_VERS', 'VERSIONE_T', $Aac_Vers_rec);
            if ($nrow == -1) {
    ...
    public function ControlliVersione($Versionte_T = '') {
        ...
        $sql = "SELECT * FROM AAC_VERS WHERE DATAINIZ = '" . $Aac_Vers_rec['DATAINIZ'] . "' ";
