...
class menExplorer extends itaModel {
    ...
    public function parseEvent() {
        ...
        switch ($_POST['event']) {
            case 'openform':
//                itaLib::openForm('menExplorer', '', true, 'desktopBody', '', 'auto', '', $this->nameForm);
                $this->OpenMenu();
                break;
    ...
    private function OpenMenu() {
        ...
        $this->albero = $this->menLib->getMenu_ini($this->rootMenu, true);
    ...
    private function dettaglioMenu($me_menu) {
        ...
        $Ita_menu_rec = $this->menLib->GetIta_menu_ini($me_menu);
        if (!$Ita_menu_rec) {
        ...
        $Ita_puntimenu_tab = $this->menLib->menuFiltrato_ini($me_menu);
...
//                $sql = "SELECT * FROM ita_menu WHERE me_menu = '" . $Ita_puntimenu_rec['pm_voce'] . "'";
                $Ita_menu_giu_rec = $this->menLib->GetIta_menu_ini($Ita_puntimenu_rec['pm_voce']);
                if ($Ita_menu_giu_rec) {
                    $Ita_puntimenu_giu_tab = $this->menLib->GetIta_puntimenu_ini($Ita_menu_giu_rec['me_menu']);
    ...
    private function clickPuntoMenu($rowid) {
        ...
        switch ($rec['pm_categoria']) {
            case 'ME':
                ...
                break;
            case 'PR':
                $Ita_menu_rec = $this->menLib->GetIta_menu_ini($rec['me_menu']);
    ...
    private function creaPercorso($rowid, $griglia) {
        ...
        if ($griglia == 'menu') {
            while (true) {
                $Ita_menu_rec = $this->menLib->GetIta_menu_ini($this->albero[$tmp]['pm_voce']);
