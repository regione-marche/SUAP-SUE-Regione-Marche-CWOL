...
function GetItaEngineContextToken($UserName, $UserPassword, $DomainCode) {
    ...
    if ($ret_verpass['status'] != 0 && $ret_verpass['status'] != '-99') {
        return new soap_fault('ERRLOGIN' . $ret_verpass['status'], '', $DomainCode . "  -- " . $ret_verpass['messaggio'], '');
    }
...
//    }
    ...
    $chiavi = array();
    $chiavi_attach = array();
    $sql = "SELECT * FROM PROPAS WHERE PRONUM='" . $CodicePratica . "' AND ((PROPUB = 1 AND (PROUPL = 1 OR PROMLT = 1 OR PRODAT = 1)) OR PROPUB=0)";
    ...
    if ($Propas_tab) {
        ...
        $Pasdoc_tab = ItaDB::DBSQLSelect($PRAM_DB, $sql, true);
        if ($Pasdoc_tab) {
            $xml = $xml . "<PASDOC>\r\n";
            foreach ($Pasdoc_tab as $Pasdoc_rec) {
                if (strtolower(pathinfo($Pasdoc_rec['PASFIL'], PATHINFO_EXTENSION)) == 'info') {
                    continue;
                }
                $xml = $xml . "<RECORD>\r\n";
                foreach ($campi as $Chiave => $Campo) {
                    $xml = $xml . "<$Campo>$cdata_a" . $Pasdoc_rec[$Campo] . "$cdata_c</$Campo>\r\n";
                }
                $xml = $xml . "</RECORD>\r\n";
            }
            $xml = $xml . "</PASDOC>\r\n";
