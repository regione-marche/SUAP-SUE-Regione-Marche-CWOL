...
function GetRichiestaDati($Token, $DomainCode, $NumeroRichiesta, $AnnoRichiesta) {
    ...
    $Proric_rec = $praLib->GetProric($CodiceRichiesta);
    if (!$Proric_rec) {
        return new soap_fault('ERRGETRICHIESTA' . 'Richiesta non disponibile', '');
    }
    $utenti_rec = $itaToken->getUtentiRec();
...
//    }
    ...
    $Ricite_tab = ItaDB::DBSQLSelect($PRAM_DB, $sql, true);
    if ($Ricite_tab) {
        $xml = $xml . "<RICITE>\r\n";
        foreach ($Ricite_tab as $Ricite_rec) {
            ...
            $xml = $xml . "<RECORD>\r\n";
            foreach ($campi as $Chiave => $Campo) {
                $xml = $xml . "<$Campo>$cdata_a" . $Ricite_rec[$Campo] . "$cdata_c</$Campo>\r\n";
            }
            $xml = $xml . "</RECORD>\r\n";
        }
        $xml = $xml . "</RICITE>\r\n";
        ...
        $campi[] = "DAGSET";
        $campi[] = "DAGVAL";
        $sql = "SELECT * FROM RICDAG WHERE ITEKEY IN (" . implode(",", $chiavi) . ") AND DAGNUM = '" . $CodiceRichiesta . "'";
        ...
        $Ricdoc_tab = ItaDB::DBSQLSelect($PRAM_DB, $sql, true);
        if ($Ricdoc_tab) {
            $xml = $xml . "<RICDOC>\r\n";
...
function GetRichiestaAllegatoForRowid($Token, $DomainCode, $rowid) {
    ...
    $Ricdoc_rec = ItaDB::DBSQLSelect($PRAM_DB, $sql, false);
    if (!$Ricdoc_rec) {
        return new soap_fault('ERRGETALLEGATO' . 'Allegato non disponibile', '');
    }
    $ext = pathinfo($dataDetail_rec['DOCUPL'], PATHINFO_EXTENSION);
    $ricPath = str_replace('$ditta', $DomainCode, Config::getPath('general.itaCms')) . "attachments/" . $Ricdoc_rec['DOCNUM'];
