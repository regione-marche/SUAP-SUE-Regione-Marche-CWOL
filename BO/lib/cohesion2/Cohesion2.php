...
class Cohesion2{
    ...
    function __construct(){
        ...
        if($this->isAuth()){
            ...
            $obj = unserialize($_SESSION[static::SESSION_NAME]);
    ...
    public function isAuth(){
    	return isset($_SESSION[static::SESSION_NAME]);
    }
    ...
    public function logout(){
        if($this->isAuth()){
            ...
            if($this->cert_file){
                $wsClient = new Cohesion2SOAP(static::COHESION2_WS.'?wsdl');
                ...
                file_get_contents(static::COHESION2_WEB,false,$context);
            }
        }
    }
    ...
    public function getCheckUrl($returnUri = false) {
        $returnUri = $returnUri ?: (($_SERVER["SERVER_PORT"] == 443) ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
        $returnUri .= (strpos($returnUri, '?') !== false ? '&' : '?') . 'cohesionCheck=1';
        ...
        $xmlAuth = '<dsAuth xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns="http://tempuri.org/Auth.xsd">
            <auth>
                ...
                <url_validate><![CDATA[' . $returnUri . ']]></url_validate>
                <url_richiesta><![CDATA[' . $returnUri . ']]></url_richiesta>
                <esito_auth_sso />
                <id_sessione_sso />
        ...
        $urlLogin = ($this->sso) ? static::COHESION2_CHECK.$auth : static::COHESION2_LOGIN.$auth;
    ...
    public function verify($auth){
        ...
        if($this->cert_file){
            ...
            $result = file_get_contents(static::COHESION2_WEB,false,$context);
            $domXML = new DOMDocument;
        ...
        if($base->login){
            ...
            $this->profile = $resp;
            $_SESSION[static::SESSION_NAME] = serialize($this);
            return true;
        }
