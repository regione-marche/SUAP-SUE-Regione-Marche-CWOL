...
class GitRepo {
	 ...
	 * @param   string  command to run
         * @param   string  $bypassShell Se true, agggiunge tra le opzioni il flag bypass_shell per windows (Aggiunto da massimob)
	 * @return  string
	 */
	protected function run_command($command, $bypassShell = false) {
		$descriptorspec = array(
			1 => array('pipe', 'w'),
			2 => array('pipe', 'w'),
		);
		...
		if(count($_ENV) === 0) {
			...
			$env = array_merge($_ENV, $this->envopts);
                ...
                // @massimob Aggiunto il parametro bypass_shell per ovviare al problema di windows con gli apici doppi e gli spazi
                if ($bypassShell) {
	 ...
	 * @return  string
	 */
	public function run($command, $bypassShell = false) {
		return $this->run_command(Git::get_bin()." ".$command, $bypassShell);
	}
	...
	public function commit($message = "", $commit_all = true) {
		...
		return $this->run("commit ".$flags." -m ".escapeshellarg($message), true);                
	}
