...
//        file_put_contents("C:/Works/PhpDev/data/itaEngine/tmp/response_$operationName.xml", $client->response);
    ...
    public function ws_LeggiProtocollo($param) {
        ...
        $param = $AnnoProtocolloSoapval->serialize() . $NumeroProtocolloSoapval->serialize() . $UtenteSoapval->serialize() . $RuoloSoapval->serialize();
...
//                }
    ...
    function ws_CreaCopieString($dati) {
        $paramArr = array();
        //
        $DataProtSoapval = new soapval('IdDocumento', 'IdDocumento', $dati['idPratica'], false, false);
        $paramArr[] = $DataProtSoapval;
        //
        $DataProtSoapval = new soapval('AnnoProtocollo', 'AnnoProtocollo', $dati['AnnoProt'], false, false);
        $paramArr[] = $DataProtSoapval;
        //
        $NumProtSoapval = new soapval('NumeroProtocollo', 'NumeroProtocollo', $dati['NumProt'], false, false);
        $paramArr[] = $NumProtSoapval;
        //
        $FascicolaConOrigSoapval = new soapval('FascicolaConOriginale', 'FascicolaConOriginale', $dati['FascConOrig'], false, false);
        $paramArr[] = $FascicolaConOrigSoapval;
        //
        if (is_array($dati['UoDestinatari'])) {
            $UODestSoapvalArr = array();
            foreach ($dati['UoDestinatari'] as $UO) {
                $UOSoapvalArr = array();
                $UOSoapvalArr[] = new soapval('Carico', 'Carico', $UO['Carico'], false, false);
                $UOSoapvalArr[] = new soapval('TipoUO', 'TipoUO', $UO['TipoUO'], false, false);
                $UOSoapvalArr[] = new soapval('Data', 'Data', $UO['Data'], false, false);
                $UOSoapvalArr[] = new soapval('NumeroCopie', 'NumeroCopie', $UO['NumeroCopie'], false, false);
                $UODestSoapvalArr[] = new soapval('UODestinataria', 'UODestinataria', $UOSoapvalArr, false, false);
            }
            $AllegatiSoapval = new soapval('UODestinatarie', 'UODestinatarie', $UODestSoapvalArr, false, false);
            $paramArr[] = $AllegatiSoapval;
        }
        //
        $utenteSoapval = new soapval('Utente', 'Utente', $this->utente, false, false);
        ...
        $paramArr[] = $ruoloSoapval;
        //
        $ProtocolloInStr = "<tem:CreaCopieInStr><![CDATA[<CreaCopieIn>";
        foreach ($paramArr as $parametro) {
            $ProtocolloInStr .= $parametro->serialize('literal');
        }
        $ProtocolloInStr .= "</CreaCopieIn>]]></tem:CreaCopieInStr>";
        $param = $ProtocolloInStr;
        //
        $CodiceAmministrazioneSoapval = new soapval('tem:CodiceAmministrazione', 'tem:CodiceAmministrazione', $this->CodiceAmministrazione, false, false);
        //
    ...
    function ws_LeggiCopie($dati) {
        ...
        $paramArr[] = $ruoloSoapval;
        //
        $ProtocolloInStr = "<tem:LeggiCopieIn><![CDATA[<LeggiCopieIn>";
        foreach ($paramArr as $parametro) {
            $ProtocolloInStr .= $parametro->serialize('literal');
        }
        $ProtocolloInStr .= "</LeggiCopieIn>]]></tem:LeggiCopieIn>";
        $param = $ProtocolloInStr;
        //
        $CodiceAmministrazioneSoapval = new soapval('tem:CodiceAmministrazione', 'tem:CodiceAmministrazione', $this->CodiceAmministrazione, false, false);
        //
        ...
        $param .= $CodiceAmministrazioneSoapval->serialize('literal');
        $param .= $CodiceAOOSoapval->serialize('literal');
        return $this->ws_call('LeggiCopie', $param);
    }

    function ws_AnnullaDocumento($dati) {
        $paramArr = array();
        //
        $DataProtSoapval = new soapval('IdDocumento', 'IdDocumento', $dati['idPratica'], false, false);
        $paramArr[] = $DataProtSoapval;
        //
        $MotivazioneSoapval = new soapval('Motivazione', 'Motivazione', $dati['Motivazione'], false, false);
        $paramArr[] = $MotivazioneSoapval;
        //
        $DataProtSoapval = new soapval('AnnoProtocollo', 'AnnoProtocollo', $dati['AnnoProt'], false, false);
        $paramArr[] = $DataProtSoapval;
        //
        ...
        $paramArr[] = $ruoloSoapval;
        //
        $ProtocolloInStr = "<tem:AnnullaDocumentoIn><![CDATA[<AnnullaDocumentoIn>";
        foreach ($paramArr as $parametro) {
            $ProtocolloInStr .= $parametro->serialize('literal');
        }
        $ProtocolloInStr .= "</AnnullaDocumentoIn>]]></tem:AnnullaDocumentoIn>";
        $param = $ProtocolloInStr;
        //
        $CodiceAmministrazioneSoapval = new soapval('tem:CodiceAmministrazione', 'tem:CodiceAmministrazione', $this->CodiceAmministrazione, false, false);
        //
        ...
        $param .= $CodiceAmministrazioneSoapval->serialize('literal');
        $param .= $CodiceAOOSoapval->serialize('literal');
        return $this->ws_call('AnnullaDocumento', $param);
    }
