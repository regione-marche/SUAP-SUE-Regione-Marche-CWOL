...
include_once dirname(__FILE__) . '/../itaPHPCore/AppUtility.class.php';
...
/* load classes
  ...
  // transport classes
  require_once('class.nusoap_transport_http.php');
 ...
 * @access   public
 * 
 * ATTENZIONE: 06/09/2016 FIX SU funzione varDump per utilizzio solo in caso di debug 
 *             per evitare saturazioni di memoria in caso di grandi xml con stream in  base64
 * @author   Mischele Moscioni <michele.moscioni@italsoft.eu>
 * 
 * 
 */
class nusoap_base {
    ...
    /**
     * If TRUE, writes a request file
     *
     * @var boolean
     * @access public
     */
    var $writeRequestFile;
    ...
    /**
     * If TRUE, writes a response file
     *
     * @var boolean
     * @access public
     */
    var $writeResponseFile;
     ...
     * HTTP conten type
     * 
     * @Michele Moscioni 29-08-2019  Proprietà per pilotare il conten type di chiamata
     * 
     * @var      string
     * @access   public
     */
    var $soap_HTTPContenType = 'text/xml';
    ...
    /**
     * @Massimo Biagioli 30-05-2018: restituisce messaggio SOAP
     * @var array
     */
    var $soapBodyAttributes = array();

    /**
     * @Mario Mazza 22-10-2019: flag per eliminare il tag SoapAction nell'header di una chiamata ricevuta dal nusoap_server
     * @var type boolean
     */
    public $clearSoapActionHeader;

    function getClearSoapActionHeader() {
        return $this->clearSoapActionHeader;
    }

    function setClearSoapActionHeader($clearSoapAction) {
        $this->clearSoapActionHeader = $clearSoapAction;
    }
    ...
    function serializeEnvelope($body, $headers = false, $namespaces = array(), $style = 'rpc', $use = 'encoded', $encodingStyle = 'http://schemas.xmlsoap.org/soap/encoding/') {
        ...
        return
                ...
                $headers .
                // @Massimo Biagioli 30-05-2018: restituisce messaggio SOAP
                (count($this->soapBodyAttributes) > 0 ? $this->getSoapBodyTagWithAttributes() : "<SOAP-ENV:Body>") .
                // @Massimo Biagioli FINE
    ...
    // @Massimo Biagioli 30-05-2018: restituisce messaggio SOAP
    private function getSoapBodyTagWithAttributes() {
        $str = '<SOAP-ENV:Body';
        foreach ($this->soapBodyAttributes as $key => $value) {
            $str .= ' ' . $key . '="' . $value . '"';
        }
        $str .= '>';
        return $str;
    ...
    function varDump($data) {
        $ret_val = "";
        /*
         * Usato solo in concomitanza del debug per ovviare a problemi di memoria MM 06/09/2016
         */
        if ($this->debugLevel > 0) {
            ob_start();
            var_dump($data);
...
class nusoap_transport_http extends nusoap_base {
     ...
     * @access public
     */
    function nusoap_transport_http($url, $curl_options = NULL, $use_curl = false) {
        parent::nusoap_base();
...
//		}
    ...
    function setSOAPAction($soapaction) {
        /*
         * @Michele Moscioni 29-08-2019: Aggiunge Soap Action Header solo se valorizzato
         * 
         * @Michele Moscioni 04-10-2019: Regressione della modifica applicata perchè alcuni servizi
         * Soap non accettano la mancanza dell'header soapAction sebbene alcuni non accetta non l'header
         * soapAction con valore Empty
         * In studio soluzione definitiva
         * 
         * 
         */
...
//		}
            ...
            //MIKI
//            $outverb = fopen('php://temp', 'w+');
//            curl_setopt($this->ch, CURLOPT_VERBOSE, true);
//            curl_setopt($this->ch, CURLOPT_STDERR, $outverb);
            // FINE MIKI
            ...
            //MIKI
//            rewind($outverb);
//            Out::msgInfo("verbose", print_r(stream_get_contents($outverb), true));
//            fclose($outverb);            
            // FINE MIKI
...
class nusoap_server extends nusoap_base {
    ...
    function parse_http_headers() {
        ...
        if (function_exists('getallheaders')) {
            ...
            if (isset($this->headers['soapaction'])) {
                if ($this->clearSoapActionHeader === true) {
                    unset($this->headers['soapaction']);
                } else {
    ...
    function invoke_method() {
        ...
        if ($appLock !== false) {
            $this->debug('Tried to invoke method ' . $this->methodname . ' while an application lock was active with the following reason: ' . $appLock['msg']);
            $this->fault('SOAP-ENV:Client', "Operation '" . $this->methodname . "' is currently unavailable for server maintenance: " . $appLock['msg']);
            return;
        }
        ...
        if (!function_exists('call_user_func_array')) {
            ...
            if ($this->methodparams) {
                ...
                $this->methodparams = $ordered_method_params;
                // --------------------------------------- //
    ...
    function getHTTPContentType() {
        // @Michele Moscioni 29-08-2019 con trollo del contentype
        return $this->soap_HTTPContenType;
        //return 'text/xml';
    }
...
class wsdl extends nusoap_base {
    ...
    function parseWSDL($wsdl = '') {
        ...
        if (isset($wsdl_props['scheme']) && ($wsdl_props['scheme'] == 'http' || $wsdl_props['scheme'] == 'https')) {
            ...
            // get wsdl
            $tr = new nusoap_transport_http($wsdl, $this->curl_options, $this->use_curl);
    ...
    function nusoap_parser($xml, $encoding = 'UTF-8', $method = '', $decode_utf8 = true) {
        ...
        if (!empty($xml)) {
            ...
            if (!empty($parseErrors)) {
                // Display an error message.
                $err = sprintf('XML error parsing SOAP payload on line %d: %s', $parseErrors['lineNumber'], $parseErrors['errorString']);
                $this->debug($err);
                $this->setError($err);
...
class nusoap_client extends nusoap_base {
     ...
     * @param   bool $returnSoapMessage If true, return only the soap message in the $soapMessage variable
     * @param   string $returnSoapMessage Soap message
     * @return	mixed	response from SOAP call, normally an associative array mirroring the structure of the XML response, false for certain fatal errors
     * @access   public
     */
    function call($operation, $params = array(), $namespace = 'http://tempuri.org', $soapAction = '', $headers = false, $rpcParams = null, $style = 'rpc', $use = 'encoded', $returnSoapMessage = false, &$soapMessage) {
        ...
        // @Massimo Biagioli 30-05-2018: restituisce messaggio SOAP
        if ($returnSoapMessage) {
            $soapMessage = $this->getHTTPBody($soapmsg);
            return;
        }
        // @Massimo Biagioli 30-05-2018: FINE
        // send        
        ...
        switch (true) {
            ...
            case preg_match('/^http/', $this->endpoint):
                ...
                if ($this->persistentConnection == true && is_object($this->persistentConnection)) {
                    ...
                    $http = new nusoap_transport_http($this->endpoint, $this->curl_options, $this->use_curl);
                    if ($this->persistentConnection) {
                ...
                // @Massimo Biagioli Salva request su file              
                if ($this->writeRequestFile === true) {
                    $msgPretty = $this->prettyXML($msg);
                    file_put_contents($this->requestFileName, $msgPretty);
                }

                $this->response = $http->incoming_payload;

                // @Massimo Biagioli Salva request su file              
                if ($this->writeResponseFile === true) {
                    $xmlPretty = $this->prettyXML($this->responseData);
                    file_put_contents($this->responseFileName, $xmlPretty);
                }

                // @Massimo Biagioli Salva fault su file              
                if ($this->writeFaultFile === true) {
                    if (isset($http->incoming_headers['x-backside-transport']) && strtoupper($http->incoming_headers['x-backside-transport']) !== 'OK OK') {
                        file_put_contents($this->faultFileName, $this->responseData);
                    }
                }
