	...
	*/
	function getHTTPBody($soapmsg) {
		if (count($this->requestAttachments) > 0) {
                        ...
                        // --- Modifica per MTOM ---
                        $params['content_type'] = 'Multipart/Related; start-info="text/xml"; type="application/xop+xml"';
			// ---
			...
			unset($params);
                        ...
                        // --- Modifica per MTOM ---
			$params['content_type'] = 'application/xop+xml; type="text/xml"';
			// ---
			...
			foreach ($structure->parts as $part) {
				if (!isset($part->disposition) && (strstr($part->headers['content-type'], 'text/xml'))) {
					...
					$root = $part->body;
                                        ...
                                        // --- FIX
                                        $newContentType = '';
                                        $headerParts = explode(';', $part->headers['content-type']);
                                        
                                        // Mette all'inizio le altre parti
                                        foreach ($headerParts as $hp) {
                                            if ((strpos($hp, 'type=') === false) && (strpos($hp, 'charset=') === false)) {
                                                if (strlen($newContentType) === 0) {
                                                    $newContentType .= $hp;
                                                } else {
                                                    $newContentType .= '; ' . $hp;
                                                }                                                
                                        ...
                                        // Cerca type
                                        foreach ($headerParts as $hp) {
                                            if (strpos($hp, 'type=') !== false) {
                                                $newContentType .= '; ' . substr($hp, strpos($hp, "=") + 1);
                                            } 
                                        }
                                        
                                        // Cerca charset
                                        foreach ($headerParts as $hp) {
                                            if (strpos($hp, 'charset=') !== false) {
                                                $newContentType .= '; ' . $hp;
                                            } 
                                        }                                        
                                        
                                        $part->headers['content-type'] = $newContentType;
					// --- END FIX
	...
	*/
	function getHTTPBody($soapmsg) {
		if (count($this->responseAttachments) > 0) {
			$params['content_type'] = 'multipart/related; type="text/xml; start="rootpart"';
			$mimeMessage = new Mail_mimePart('', $params);
			unset($params);
