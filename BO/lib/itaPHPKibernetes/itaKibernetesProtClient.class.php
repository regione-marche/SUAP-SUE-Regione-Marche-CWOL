...
//        file_put_contents("F:/works/tmp/response_kibernetes_$operationName.xml", $client->response);
    ...
    public function ws_Set4ProtocolloUscita($dati) {
        $IstatSoapval = new soapval('istatEnte', 'istatEnte', $this->istatAmministrazione, false, false);
        $MittenteSoapval = new soapval('destinatario', 'destinatario', $dati['Destinatario'], false, false);
        $IndirizzoSoapval = new soapval('indirizzo', 'indirizzo', $dati['Indirizzo'], false, false);
        $OggettoSoapval = new soapval('oggetto', 'oggetto', $dati['Oggetto'], false, false);
        $UOSoapval = new soapval('UO', 'UO', $this->CodiceUOPar, false, false);

        $DestinatarioSecSoapval = new soapval('destinatarioSec', 'destinatarioSec', $dati['DestinatarioSec'], false, false);
        $IndirDestinatarioFunzSecSoapval = new soapval('indirizzoDestinatarioSec', 'indirizzoDestinatarioSec', $dati['IndirDestinatarioSec'], false, false);
        $DestinatarioSecCCSoapval = new soapval('destinatarioSecCC', 'destinatarioSecCC', $dati['DestinatarioSecCC'], false, false);

        $FunzionarioSoapval = new soapval('funzionario', 'funzionario', $this->funzionarioPar, false, false);
        $AnnoPrecSoapval = new soapval('annoAntecedente', 'snnoAntecedente', $dati['AnnoPrec'], false, false);
        $ProtPrecSoapval = new soapval('numeroAntecedente', 'numeroAntecedente', $dati['ProtPrec'], false, false);

        $param = $IstatSoapval->serialize('literal') . $MittenteSoapval->serialize('literal') . $IndirizzoSoapval->serialize('literal') . $OggettoSoapval->serialize('literal') . $UOSoapval->serialize('literal') .
                $DestinatarioSecSoapval->serialize('literal') . $IndirDestinatarioFunzSecSoapval->serialize('literal') . $DestinatarioSecCCSoapval->serialize('literal') .
                $FunzionarioSoapval->serialize('literal') . $AnnoPrecSoapval->serialize('literal') . $ProtPrecSoapval->serialize('literal');

        return $this->ws_call('set4ProtocolloUscita', $param);
    }

    public function ws_Set4ProtocolloInterno($dati) {
        $IstatSoapval = new soapval('istatEnte', 'istatEnte', $this->istatAmministrazione, false, false);
        $MittenteSoapval = new soapval('mittente', 'mittente', $dati['Mittente'], false, false);
        $IndirizzoSoapval = new soapval('indirizzo', 'indirizzo', $dati['Indirizzo'], false, false);
        $OggettoSoapval = new soapval('oggetto', 'oggetto', $dati['Oggetto'], false, false);
        $UOSoapval = new soapval('UO', 'UO', $this->CodiceUOPar, false, false);
        $DestinatarioUOSoapval = new soapval('destinatarioUO', 'destinatarioUO', $dati['DestinatarioUO'], false, false);
        $DestinatarioFunzSoapval = new soapval('destinatarioFunz', 'destinatarioFunz', $dati['FunzionarioDest'], false, false);

        $DestinatarioUOSecSoapval = new soapval('destinatarioSecUO', 'destinatarioSecUO', $dati['UfficioSec'], false, false);
        $DestinatarioFunzSecSoapval = new soapval('destinatarioSecFunz', 'destinatarioSecFunz', $dati['FunzionarioDestSec'], false, false);
        $DestinatarioSecCCSoapval = new soapval('secondarioCC', 'destinatarioSecCC', $dati['FunzionarioDestSecCC'], false, false);
        //$DestinatarioSecCCSoapval = new soapval('destinatarioSecCC', 'destinatarioSecCC', $dati['FunzionarioDestSecCC'], false, false);
        ...
        return $this->ws_call('set4ProtocolloInterno', $param);
    }

    public function ws_Search4Funzionario($dati) {
        $IstatSoapval = new soapval('istatEnte', 'istatEnte', $this->istatAmministrazione, false, false);
        $DescrizioneSoapval = new soapval('descrizione', 'descrizione', $dati['Descrizione'], false, false);
        $param = $IstatSoapval->serialize('literal') . $DescrizioneSoapval->serialize('literal');
        return $this->ws_call('search4Funzionario', $param);
    }

    public function ws_Check4Funzionario($funzionario) {
        $IstatSoapval = new soapval('istatEnte', 'istatEnte', $this->istatAmministrazione, false, false);
        $DescrizioneSoapval = new soapval('descrizione', 'descrizione', $funzionario, false, false);
        $param = $IstatSoapval->serialize('literal') . $DescrizioneSoapval->serialize('literal');
        return $this->ws_call('Check4Funzionario', $param);
    }
