	...
	*/
	function getHTTPBody($soapmsg) {
		if (count($this->requestAttachments) > 0) {
			$params['content_type'] = 'multipart/related; type="text/xml"; start="'.htmlspecialchars('<rootpart>').'"';
			...
			$params['charset']      = $this->soap_defencoding;
                        $params['cid']          = "rootpart";//$params['cid']          = "rootpart"; //MM
			$mimeMessage->addSubpart($soapmsg, $params);
			
			foreach ($this->requestAttachments as $att) {
				unset($params);
                                $params=array();    
				$params['content_type'] = "application/octet-stream";//$att['contenttype'];
				$params['encoding']     = 'binary'; //MM
				//$params['disposition']  = 'attachment'; //MM
				//$params['dfilename']    = $att['filename']; /MM
				$params['cid']          = "attachment";//$att['cid'];
				...
				if ($att['data'] == '' && $att['filename'] <> '') {
					...
					$mimeMessage->addSubpart($att['data'], $params);
                                ...
                                foreach ($params as $key => $value) {
                                   $this->debug("PARM: ".$key." VALORE: ".$value); 
                                }
			}
