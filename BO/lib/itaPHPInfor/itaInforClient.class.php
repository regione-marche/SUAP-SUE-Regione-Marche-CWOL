...
class itaInforClient {
    ...
    //private $namespace = "http://infor.arezzo.it/jente/jbase/services";
    private $namespace = "";
    ...
    private function ws_call($operationName, $param) {
        ...
        //file_put_contents("/users/tmp/request_infor.xml", $client->request);
        //file_put_contents("/users/tmp/response_infor.xml", $client->response);
        if ($client->fault) {
    ...
    public function ws_serviziRicerca($ServiziRicerca) {
        return $this->ws_call($this->namespacePrefix . ":serviziRicerca", $ServiziRicerca->getRichiesta($this->namespacePrefix));
    }
