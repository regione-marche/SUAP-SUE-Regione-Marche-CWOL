 ...
 * mime_parser.php
 *
 * @(#) $Id: mime_parser.php,v 1.70 2010/07/08 01:42:16 mlemos Exp $
 *
 */
...
/*
  ...
  <version>@(#) $Id: mime_parser.php,v 1.70 2010/07/08 01:42:16 mlemos Exp $</version>
...
class mime_parser_class {
    ...
    Function ParseStructuredHeader($value, &$type, &$parameters, &$character_sets, &$languages) {
        ...
        while (strlen($p)) {
            ...
            /*
             * Agggiunto preg_match per controllo caratteri protetti all'interno del valore
             * Calcola la posizione dei doppi apici validi.
             * 
             * 
             */
            preg_match('/".+?(?<!\\\\)(")/', $remaining, $matches, PREG_OFFSET_CAPTURE);
            //if (strlen($remaining) && !strcmp($remaining[0], '"') && (GetType($quote = strpos($remaining, '"', 1)) == 'integer')) {
            if (strlen($remaining) && !strcmp($remaining[0], '"') && (GetType($quote = $matches[1][1]) == 'integer')) {
...
//                    $value = ($len >0) ? str_repeat('\'', $len) . $value : $value;
            ...
            /*
             * Parsing dei parametri nel formato "key*n". (es. name*0, name*1, ecc.)
             */
            if (($l = strlen($parameter)) && !strcmp($parameter[$l - 2], '*')) {
                $parameter = $this->Tokenize($parameter, '*');
                if (IsSet($parameters[$parameter]) && IsSet($character_sets[$parameter]))
                    $value = $parameters[$parameter] . UrlDecode($value);
                else {
            ...
            case MIME_PARSER_HEADER:
                ...
                if (GetType($colon = strpos($this->buffer, ':', $this->buffer_position)) == 'integer') {
                    if (GetType($space = strpos(substr($this->buffer, $this->buffer_position, $colon - $this->buffer_position), ' ')) == 'integer') {
                        if ((!$this->mbox || strcmp(substr($this->buffer, $this->buffer_position, $space), 'From')) && !$this->SetPositionedWarning('invalid header name line', $this->buffer_position))
                            return(0);
        ...
        for ($message = 0; ($success = $this->DecodeStream($parameters, $end_of_message, $decoded_message)) && !$end_of_message; $message++) {
            if ($this->extract_addresses) {
                ...
                for (Reset($headers), $h = 0; $h < $th; Next($headers), ++$h) {
                    $header = Key($headers);
                    if (IsSet($this->address_headers[$header]) && $this->address_headers[$header]) {
