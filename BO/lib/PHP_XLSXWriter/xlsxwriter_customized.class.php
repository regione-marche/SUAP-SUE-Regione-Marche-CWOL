...
class XLSXWriter {
    ...
    public function writeSheetRow($sheet_name, array $row, $row_options = null) {
        ...
        $numberFormatArray = array();
        $numberFormatType = array();
        if (!empty($row_options)) {
            ...
            if(!empty($row_options['format'])){
                foreach($row_options['format'] as $format){
                    if(empty($format)){
                        $numberFormatArray[] = null;
                        $numberFormatType[] = null;
                    }
                    else{
                        $number_format = self::numberFormatStandardized($format);
                        $numberFormatArray[] = $number_format;
                        $numberFormatType[] = self::determineNumberFormatType($number_format);
                    }
                }
                unset($row_options['format']);
            }
//            $sheet->file_writer->write('<row collapsed="' . ($collapsed) . '" customFormat="false" customHeight="' . ($customHt) . '" hidden="' . ($hidden) . '" ht="' . ($ht) . '" outlineLevel="0" r="' . ($sheet->row_count + 1) . '">');
        ...
        $c = 0;
        foreach ($row as $v) {
            $number_format = (isSet($numberFormatArray[$c]) ? $numberFormatArray[$c] : $sheet->columns[$c]['number_format']);
            $number_format_type = (isSet($numberFormatArray[$c]) ? $numberFormatType[$c] : $sheet->columns[$c]['number_format_type']);
