...
require_once ITA_LIB_PATH . '/itaPHPDocs/itaIterableData.class.php';
...
class itaDictionary {
    ...
    public function importDataFromXml($xmlString) {
        ...
        $xmlObj = new itaXML;
        $xmlObj->setTrimBlanks($this->trimBlanks);
...
//                $arrXml[$key] = self::filterTextNode($value[0]);
     ...
     * @return array
     */
    public function getAllData($encoding = '') {
        $retData = array();
        foreach ($this->data as $key => $value) {
            if (is_object($value)) {
                if (get_class($value) == 'itaDictionary') {
                    $retData[$key] = $value->getAllData($encoding); //mm
                } else if (get_class($value) == 'itaDocumentDOCX') {    // TODO: parlare con Carlo e Michele per la merge
                    $retData[$key] = $value;
                } else if (in_array('itaIterableDataInterface', class_implements($value))) {
                    $retData[$key] = $value;
                } else {
                    $retData[$key] = '';
                }
                ...
                if ($encoding != '') {
                    if ($encoding == 'UTF-8') {
                        $retData[$key] = htmlspecialchars(utf8_encode($value), ENT_COMPAT, $encoding);
                    } else {
                        $retData[$key] = htmlspecialchars($value, ENT_COMPAT, $encoding);
                    }
                } else {
                    $retData[$key] = $value;
                }
    ...
    public function getAllDataFormatted() {
        ...
        foreach ($this->data as $key => $value) {
            if (is_object($value)) {
                ...
                } else if (get_class($value) == 'itaDocumentDOCX') {
                    $retData[$key] = $value;
                } else if (in_array('itaIterableDataInterface', class_implements($value))) {
                    $retData[$key] = $value;
                } else {
            ...
            if ($this->dictionary[$wKey]['format']) {
                ...
                switch ($arrf['type']) {
                    ...
                    case 'number':
                        ...
                        break;
                    case 'html':
                        $xvalue = ($value) ? "Content-type: text/html\n" . $value : "";
                        break;
                    case 'qr':
                        $xvalue = ($value) ? $this->getQRCode($value) : '';
                        break;
                    default:
                        $xvalue = $value;
                        break;
    ...
    private function getQRCode($data) {
        ...
        $filetemp = itaLib::getAppsTempPath() . DIR_SEP . uniqid() . '.png';
        QRcode::png($data, $filetemp, QR_ECLEVEL_L, 10, 2);

        $b64 = itaImg::base64src($filetemp);
        unlink($filetemp);
    ...
    public function getAllDataXML() {
        ...
        foreach ($this->data as $key => $value) {
            if (is_object($value)) {
                ...
                $stringa = "<" . $key . ">" . htmlspecialchars(utf8_encode($value), ENT_COMPAT, 'UTF-8') . "</" . $key . ">\n"; //caso di default
                if (isset($this->dictionary[$key]['format']) && $this->dictionary[$key]['format']) {
                    $arrf = json_decode("{" . $this->dictionary[$key]['format'] . "}", true);
                    switch ($arrf['type']) {
                        case 'html':
                            $stringa = "<" . $key . "><![CDATA[" . utf8_encode($value) . "]]></" . $key . ">\n";
                            break;
                    }
                }
                $retXml .= $stringa;
    ...
    public function getAllDataJSON() {
        $array = $this->getAllDataFormatted();
        return json_encode($array);
    }

    public function getDictionaryJSON($markup = 'smarty', $rootDescription = 'Legenda campi') {
//        $array = array();
//
//        foreach ($this->dictionary as $key => $field) {
//            if (is_object($this->data[$key])) {
//                switch (get_class($this->data[$key])) {
//                    case 'itaDictionary':
//                        $array[$key] = json_decode($this->data[$key]->getDictionaryJSON(), true);
//                        break;
//
//                    default:
//                        $array[$key] = null;
...
//            }
//
//            $array[$key] = is_array($field) ? array_map('utf8_encode', $field) : utf8_encode($field);
//        }
//
//        return json_encode($array);
        ...
        $arrayJSON[$i]['varidx'] = $i;
        $arrayJSON[$i]['chiave'] = '';
        $arrayJSON[$i]['markupkey'] = '';
        $arrayJSON[$i]['descrizione'] = $rootDescription;
        $arrayJSON[$i]['valore'] = '';
        $arrayJSON[$i]['level'] = $level;
        $arrayJSON[$i]['parent'] = 'null';
        $arrayJSON[$i]['isLeaf'] = 'false';
        ...
        foreach ($this->dictionary as $dictKey => $dictField) {
            $i = count($arrayJSON);
            $arrayJSON[$i]['varidx'] = $i;
            $arrayJSON[$i]['chiave'] = $dictKey;
            $arrayJSON[$i]['markupkey'] = $this->createKey($dictKey, $markup);
            $arrayJSON[$i]['descrizione'] = $dictField['desc'];
            $arrayJSON[$i]['type'] = $dictField['type'];
            $arrayJSON[$i]['level'] = $level;
            $arrayJSON[$i]['parent'] = 0;
            $arrayJSON[$i]['isLeaf'] = 'false';

            $save_count = count($arrayJSON);

            if ($dictField['type'] == 'itaDictionary') {
                $arrayJSON[$i]['markupkey'] = '';
                $arrayJSON = $this->explodeDictionaryJSON($this->getData($dictKey), $arrayJSON, $level + 1, $i, $markup);
                $arrayJSON[$i]['valore'] = '';
            } else {
                $arrayJSON[$i]['valore'] = $this->getData($dictKey);
            }

            if ($save_count == count($arrayJSON)) {
                $arrayJSON[$i]['descrizione'] = $dictField['desc'];
                $arrayJSON[$i]['isLeaf'] = 'true';
            }
        }

        foreach ($arrayJSON as $k => $obj) {
            $arrayJSON[$k] = array_map('utf8_encode', $obj);
        }
    ...
    private function explodeDictionaryJSON($dictionary, $albero, $lev, $parent, $markup) {
        if ($lev == 30) {
            return $albero;
        }
        ...
        foreach ($dictionary->dictionary as $dictKey => $dictField) {
            ...
            if ($dictField['type'] == 'itaDictionary') {
                $albero[$inc]['markupkey'] = "";
                $albero = $this->explodeDictionaryJSON($dictionary->getData($dictKey), $albero, $lev + 1, $inc, $markup);
                $albero[$inc]['valore'] = "";
            } else {
                $albero[$inc]['valore'] = $this->getData($albero[$parent]['chiave'] . "." . $dictKey);
            }
