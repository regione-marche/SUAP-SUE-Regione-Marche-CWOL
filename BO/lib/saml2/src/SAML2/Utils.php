...
class Utils
{
     ...
     * @param bool $toReturn Se true, restituisce la signature in $returnData
     * @param array $references References su cui generare i digest values
    ...
    public static function insertSignature(
        ...
        if (count($references) > 0) {
            ...
            foreach ($references as $reference) {
                $elements = $root->getElementsByTagNameNS($reference['ns'], $reference['tagName']);
                
                // @massimob: bugfix per versioni di php < 5.6
                //$nodes[] = $elements[0];        
                $nodes[] = $elements->item(0);                        
            }
            $objXMLSecDSig->addReferenceList(
                $nodes,
                $type,
                array('http://www.w3.org/2000/09/xmldsig#enveloped-signature', XMLSecurityDSig::EXC_C14N),
                array('id_name' => 'wsu:Id', 'overwrite' => false)
            );     
        } else {
            $objXMLSecDSig->addReferenceList(
                array($root),
        ...
        $objXMLSecDSig->insertSignature($root, $insertBefore, $toReturn, $returnData);
    }
