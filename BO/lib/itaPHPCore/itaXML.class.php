...
// Correzione su gestione chiave array. $j rappresenta un valore numerico e non è una chiave dell'array $altKeyXml.
// Probabilmente è stata fatta confusione fra le due forme foreach($altKeyXml as $j => $p) e foreach(array_keys($altKeyXml) as $j)
//
//                        foreach (array_keys($altKeyXml) as $j) {
    ...
    public static function validateXml($file = null, $filexsd = null) {
        ...
        if (!$file) {
            $result["status"] = false;
            $warnings["key"] = -1;
            $warnings["message"] = "Precondition: il file xml " . $file . "non è valido";
            return $result;
        }
        ...
        if (!$filexsd) {
            $result["status"] = false;
            $warnings["key"] = -2;
            $warnings["message"] = "Precondition: il file xsd " . $filexsd . "non è valido";
            return $result;
        }
