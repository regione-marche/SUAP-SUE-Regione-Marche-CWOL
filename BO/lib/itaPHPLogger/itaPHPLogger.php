...
class itaPHPLogger extends Psr\Log\AbstractLogger {
    ...
    /**
     * Richiama il metodo close del logger che pulisce il buffer
     * degli handler.
     */
    ...
    /**
     * Aggiunge l'handler di log tramite Email con buffer (una singola mail con
     * tutti i log viene smistata a fine vita del logger).
     * 
     * @param integer $level   Livello minimo dei log da registrare con questo handler.
     * @param boolean $bubble  Se 'false', i log si fermeranno con questo handler.
     * @return \itaPHPLogger
     */
    public function pushBufferEmail($level = self::DEBUG, $bubble = true, $toAddress = '') {
        $emailHandler = new EmailHandler($level, $bubble, $toAddress);
        $bufferHandler = new \Monolog\Handler\BufferHandler($emailHandler, 0, $level, $bubble);
