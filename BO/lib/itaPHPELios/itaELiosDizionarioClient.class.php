...
//        Out::msgInfo("operation", $operationName);
//        Out::msgInfo("soapAction", $soapAction);
//        Out::msgInfo("uri", $this->webservices_uri);
//        return true;
        $headers = false;
        $rpcParams = null;
        $style = 'rpc';
        ...
        file_put_contents('/users/tmp/paramDiz.xml', $param);
        file_put_contents('/users/tmp/requestDiz.xml', $client->request);
        file_put_contents('/users/tmp/responseDiz.xml', $client->response);
        if ($client->fault) {
    ...
    public function ws_ListaCodiceTitolario($param) {
        $strUserNameSoapval = new soapval('tem:strUserName', 'tem:strUserName', $param['strUserName'], false, false);
        $strTokenSoapval = new soapval('tem:strDST', 'tem:strDST', $param['strDST'], false, false);
        $param = $strUserNameSoapval->serialize("literal") . $strTokenSoapval->serialize("literal");
        return $this->ws_call('ListaCodiceTitolarioRequest', $param);
    }

    public function ws_ListaUffici($param) {
        $strUserNameSoapval = new soapval('tem:strUserName', 'tem:strUserName', $param['strUserName'], false, false);
        $strTokenSoapval = new soapval('tem:strDST', 'tem:strDST', $param['strDST'], false, false);
        $param = $strUserNameSoapval->serialize("literal") . $strTokenSoapval->serialize("literal");
        return $this->ws_call('ListaUfficiRequest', $param);
    }
