...
class pop3_class {
    ...
    Function OpenConnection() {
        if($this->pop3_secure == 'ssl' || $this->pop3_secure == 'tls'){
            $version=explode(".",function_exists("phpversion") ? phpversion() : "3.0.7");
            $php_version=intval($version[0])*1000000+intval($version[1])*1000+intval($version[2]);
            if($php_version<4003000)
                return("establishing TLS or SSL connections requires at least PHP version 4.3.0");
            if(!function_exists("extension_loaded")
                    || !extension_loaded("openssl"))
                return("establishing TLS or SSL connections requires the OpenSSL extension enabled");
        }
        ...
        $error=$error_message='';
        if(($this->connection=@fsockopen(($this->pop3_secure ? $this->pop3_secure. "://" : "").$this->hostname, $this->port, $error, $error_message)) == 0) {
            switch($error) {
                case -3:
...
/**
 * Classe deprecata per l'utilizzo esclusivo di protocollo tsl
 */
class pop3_stream {
    var $opened = 0;
    var $report_errors = 1;
