...
class itaWsFascicolazioneClient {
    ...
    private function ws_call($operationName, $param, $ns = "tem:") {
        ...
        file_put_contents("/users/tmp/IRparam_$operationName.xml", $param);
        file_put_contents("/users/tmp/IRrequest_$operationName.xml", $client->request);
        file_put_contents("/users/tmp/IRresponse_$operationName.xml", $client->response);
        $time = time();
        if ($client->fault) {
