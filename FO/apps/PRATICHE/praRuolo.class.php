...
class praRuolo {
    ...
    /**
     * 
     * @param type $praLib Libreria di lavoro 
     */
    static public function initSistemSubjectRoles($praLib) {
        foreach (self::$SISTEM_SUBJECT_ROLES as $Ruolo) {
            $Anaruo_rec = $praLib->GetAnaruo($Ruolo['RUOCOD']);
            if (!$Anaruo_rec) {
                $Anaruo_rec = $praLib->SetMarcaturaRuolo($Ruolo, true);
                try {
                    $nrow = ItaDB::DBInsert($praLib->getPRAMDB(), "ANARUO", 'ROWID', $Anaruo_rec);
                    if ($nrow != 1) {
                        return false;
                    }
                } catch (Exception $exc) {
