...
class praMup extends itaModelFO {
...
//                if ($this->praLib->getDataScadenza($Proric_rec))
...
//                }
                ...
                if ($Ricite_rec['ITEUPL'] != 0 || $Ricite_rec['ITEMLT'] != 0) {
                    ...
                    $msgTariffa = $this->praLib->checkTariffa($dati);
                    if ($msgTariffa) {
                    ...
                    $html = new html();
                    /*
                     * Se è una raccolta e c'è l'XHTML creo il pdf dal testo base
                     */
                    if (($dati['Ricite_rec']['ITEDAT'] == 1 || $dati['Ricite_rec']['ITERDM'] == 1) && $dati['Ricite_rec']['ITEMETA']) {
                        if ($dati['Ricite_rec']['ITEMETA']) {
                            ...
                            if ($metadati === false) {
                                output::addAlert('Impossibile estrapolare il template del testo.', 'Errore', 'error');
                            ...
                            if (isset($metadati["TESTOBASEXHTML"])) {
                                $pdfPreview = $this->praLib->CreaPdfRaccolta($dati, $this->PRAM_DB);
                                if (!$pdfPreview) {
                                    output::addAlert('Impossibile creare il PDF.', 'Errore', 'error');
                                    $dati = $this->praLibDati->prendiDati($dati['Proric_rec']['RICNUM'], $this->request['seq']);
                                    break;
                                }
                                $raccolta_pdfa = $this->praLib->SalvaPdfRaccolta($pdfPreview, $dati, $this->PRAM_DB);
                                if (!$raccolta_pdfa) {
                                    output::addAlert('Impossibile salvare il PDF.', 'Errore', 'error');
                                    $dati = $this->praLibDati->prendiDati($dati['Proric_rec']['RICNUM'], $this->request['seq']);
                                    break;
                                }

                                $origName = pathinfo($raccolta_pdfa, PATHINFO_BASENAME);
                                $Ricdoc_rec = ItaDB::DBSQLSelect($this->PRAM_DB, "SELECT * FROM RICDOC WHERE DOCNUM = '" . $dati['Proric_rec']['RICNUM'] . "' AND ITEKEY = '" . $dati['Ricite_rec']['ITEKEY'] . "' AND DOCUPL = '" . pathinfo($raccolta_pdfa, PATHINFO_BASENAME) . "'", false);
                                if ($Ricdoc_rec) {
                                ...
                                $datiAllegato = $this->getPercorsoAllegato(pathinfo($raccolta_pdfa, PATHINFO_BASENAME), $dati, $origName);
                    ...
                    output::$html_out .= $html->getHtml();
                } elseif ($dati['Ricite_rec']['ITEDISCOMUNICA'] != 0) {
                    $html = new html();
                    if ($this->request['ITA_USERTELEMACO'] == "" || $this->request['ITA_CFTELEMACO'] == "") {
                        $html->appendHtml("<div class=\"ita-alert\" title=\"Creazione Distinta Infocamere\">
                                                    <p style=\"padding:5px;color:red;font-size:1.2em;\"><b>I 2 campi non possono essere vuoti.<br>Compilarli entambi per andare avanti.</b></p>
                                              </div>");
                        output::$html_out .= $html->getHtml();
                        break;
                    }

                    if (!$this->praLibDatiAggiuntivi->controllaValidoSe('CodiceFiscale', $this->request['ITA_CFTELEMACO'])) {
                        $html->addAlert('Il campo <b>Codice Fiscale per Starweb</b> contiene un valore non valido.');
                        output::$html_out .= $html->getHtml();
                        break;
                    }


                    $dati['ita_usertelemaco'] = $this->request['ITA_USERTELEMACO'];
                    $dati['ita_cftelemaco'] = strtoupper($this->request['ITA_CFTELEMACO']);
                    ...
                    if ($dati['Proric_rec']['CODICEPRATICASW'] == "") {
                        /*
                         * Creazione cartella temporanea contenente tutti i file occorrenti da mandare a COMUNICA
                         */
                        $codicePratica = $praLibInfocamere->creaPratica($dati);
                    } else {
                        /*
                         * Caricamento cartella temporanea contenente tutti i file occorrenti da mandare a COMUNICA
                         */
                        $codicePratica = $praLibInfocamere->caricaPratica($dati);
                    }

                    /*
                     * Controllo esistenza codice pratica
                     */
                    if (!$codicePratica) {
                        if ($praLibInfocamere->getErrCode() == "W0099") {
                            $html->appendHtml("<div class=\"ita-alert\" title=\"Creazione Distinta Infocamere\">
                                                    <p style=\"padding:5px;color:red;font-size:1.2em;\"><b>" . $praLibInfocamere->getErrMessage() . "</b></p>
                                              </div>");
                            output::$html_out .= $html->getHtml();
                            break;
                        } else {
                            return output::$html_out = $this->praErr->parseError(__FILE__, 'E0070', $praLibInfocamere->GetErrMessage(), __CLASS__);
                        }
                    }

                    /*
                     * Ricarico i dati per allineare il codicepratica
                     */
                    ...
                    /*
                     * Scarico il pdf della distinta
                     */
                    $downloadURI = $this->frontOfficeLib->getDownloadURI($praLibInfocamere->getCartellaZIP() . "/" . $praLibInfocamere->getFileNameDistinta(), $praLibInfocamere->getFileNameDistinta());
                    $html->addJSRedirect($downloadURI);
            ...
            case 'invioInfocamere':
                ...
                if ($this->request['TIPOLOGIA_SEGNALAZIONE'] == "") {
                    output::addAlert('Tipologia segnalazione mancante.', 'Errore', 'error');
                    break;
                } else {
                    $dati['dati_infocamere']["datiAdempimento"]["tipologia_segnalazione"] = $this->request['TIPOLOGIA_SEGNALAZIONE'];
                    $dati['dati_infocamere']["datiAdempimento"]["oggetto_comunicazione"] = $this->request['TIPOLOGIA_SEGNALAZIONE'];
                }
                ...
                if (ITA_JVM_PATH != "" && file_exists(ITA_JVM_PATH) && file_exists(ITA_LIB_PATH . "/java/itaStarWebWS.properties")) {
                    ...
                    if ($arrayExec[1] == 0) {
                        ...
                        $arrayNote['INFOCAMERE']['TIPOLOGIA_SEGNALAZIONE'] = $datiInfocamere["datiAdempimento"]["tipologia_segnalazione"];
                        ...
                        /*
                         * Preparo gli allegati da inviare per mail 
                         */
                        ...
                        if ($dati['Ricite_rec']['ITEMRI'] == 0) {
                            if ($arrayParamBloccoMail['bloccaMailRich'] == null || $arrayParamBloccoMail['bloccaMailRich'] == "No") {
                                ...
                                /*
                                 * Swicchato if per segnalazione Comunità alto medio metauro il 28/12/2016.
                                 * Entrava sempre nell'if perche la funzione InvioMailRichiedente tornava true quindi 1 quindi $ErrorMail risultava valorizzato
                                 */
                                if ($ErrorMail != 1) {
                                    //if ($ErrorMail) {
                                    $msgErrResp = "Impossibile inviare momentaneamente la mail relativa alla richiesta n. " . $dati['Proric_rec']['RICNUM'] . " al richiedente.<b>
                                           ...
                                           Se il problema persiste contatatre l'assistenza software.<br>
                                           LA RICHIESTA E' STATA COMUNQUE INVIATA AD INFOCAMERE";
            ...
            case 'invioInfocamereNEW':
                /*
                 * Carico risorse e Parametri
                 */
                $html = new html();
                if (!$dati = $this->praLibDati->prendiDati($this->request['ricnum'], $this->request['seq'])) {
                    return $this->praLibDati->gestioneErrore($this->praLibDati->getErrCode(), $this->praLibDati->getErrMessage(), $this->praErr);
                 ...
                 * Creazione file zip
                 */
                if (!$praLibInfocamere->creaFileZIP()) {
                    return output::$html_out = $this->praErr->parseError(__FILE__, $praLibInfocamere->getErrCode(), "Errore nella creazione dello zip --> " . $praLibInfocamere->GetErrMessage(), __CLASS__);
                }
                ...
                /*
                 * Creo il file xml info principale
                 */
                $Nome_file = $dati['CartellaAllegati'] . "/XMLINFO.xml";
                $File = fopen($Nome_file, "w+");
                if (!file_exists($Nome_file)) {
                 ...
                 */
                $anaparInvioSW_rec = $this->praLib->GetAnapar("BLOCK_INVIO_STARWEB", "parkey", $this->PRAM_DB, false);

                /*
                 * Invio zip a infocamere
                 */
                if ($anaparInvioSW_rec['PARVAL'] == "No" || $anaparInvioSW_rec['PARVAL'] == "") {
                    $retInvio = $praLibInfocamere->inviaZipWsComunica();
                    if (!$retInvio) {
                        $retCancella = $praLibInfocamere->cancellaFileZIP();
                        if (!$retCancella) {
                            $this->praErr->parseError(__FILE__, $praLibInfocamere->getErrCode(), $praLibInfocamere->getErrMessage(), __CLASS__);
                            output::$html_out .= $html->getHtml();
                            return output::$html_out;
                        }
                        $html->appendHtml("<script type=\"text/javascript\">alert(\"" . $praLibInfocamere->getErrMessage() . "\");</script>");
                        $this->praErr->parseError(__FILE__, $praLibInfocamere->getErrCode(), $praLibInfocamere->getErrMessage(), __CLASS__, "", false);
                        output::$html_out .= $html->getHtml();
                        return output::$html_out;
                    }
                    $operaz = "inviata";
                     ...
                     * Scarico lo zip
                     */
                    $downloadURI = $this->frontOfficeLib->getDownloadURI($dati['CartellaAllegati'] . "/" . $dati['Proric_rec']['CODICEPRATICASW'] . ".zip", $dati['Proric_rec']['CODICEPRATICASW'] . ".SUAP.zip");
                    $html->addJSRedirect($downloadURI);
                ...
                /*
                 * Salvataggio log invio
                 */
                $arrayNote = unserialize($dati['Ricite_rec']['RICNOT']);
                $arrayNote['NOTE'] = "la richiesta " . $praLibInfocamere->getCodicePratica() . " e' stata $operaz in data " . date('d/m/Y');
                $arrayNote['INFOCAMERE']['FILENAME'] = $praLibInfocamere->getFileNameZIP();
                $arrayNote['INFOCAMERE']['DATE'] = date('dmY');
                $arrayNote['INFOCAMERE']['TIPOLOGIA_SEGNALAZIONE'] = $datiInfocamere["datiAdempimento"]["tipologia_segnalazione"];
                $dati['Ricite_rec']['RICNOT'] = serialize($arrayNote);
                try {
            ...
            case 'cancellaAllegato':
                ...
                if (strtolower(pathinfo($this->request['allegato'], PATHINFO_EXTENSION)) == "pdf" && $Ricdag_tab) {
                    ...
                    if ($effettuaCheckAnnullaRaccolta && $arrWarning) {
                        $href = ItaUrlUtil::GetPageUrl(array('allegato' => $this->request['allegato'], 'event' => 'confermaCancellaAllegato', 'seq' => $dati['seq'], 'ricnum' => $dati['Proric_rec']['RICNUM']));
                ...
                /*
                 * Verifica dipendenze per distinta infocamere
                 */
                if ($dati['Ricite_rec']['ITEIFC']) {
                    require_once ITA_PRATICHE_PATH . '/PRATICHE_italsoft/praLibInfocamere.class.php';
                    $praLibInfocamere = new praLibInfocamere($this->praLib);
                     ...
                     */
                    $statoRapportoDistinta = $praLibInfocamere->getStatoDistintaInfocamere();

                    // 1. Distinta Infocamere
                    if ($statoRapportoDistinta['PassoEseguito']) {
                        $msgRap = "La Distinta infocamere sarà annullata e dovrà essere ricreata";
                        if ($statoRapportoDistinta['UploadCaricato'] == 1) {
                            $msgRap .= " e ricaricata";
                        }
                        if ($msgRap) {
                            $arrWarning[] = array(
            ...
            case 'confermaCancellaAllegato':
                ...
                if (strtolower(pathinfo($this->request['allegato'], PATHINFO_EXTENSION)) == "pdf") {
                    ...
                    if ($Ricite_tab_default) {
                        ...
                        /*
                         * Cancello la distinta se presente, perchè se ci sono dipendenze, l'allegato cancellato farà parte del rapporto
                         * e il rapporto farà parte dello zip infocamere
                         */
                        if (!$praLibInfocamere->cancellaPratica()) {
                            $html->appendHtml("<div style=\"display:none\" class=\"ita-alert\" title=\"Cancellazione Distinta!\">
                                                  <p style=\"padding:5px;color:red;font-size:1.2em;\"><b>" . $praLibInfocamere->getErrMessage() . "</b></p>
                                              </div>");
                            output::$html_out .= $html->getHtml();
                            break;
                ...
                /*
                 * Cancello distinta infocamere se è un passo che va nello zip
                 */
                if ($dati['Ricite_rec']['ITEIFC']) {
                    /*
                     * Carico i dati della pratica 
            ...
            case 'annullaRaccolta':
                ...
                if ($effettuaCheckAnnullaRaccolta && $arrWarning) {
                    $href = ItaUrlUtil::GetPageUrl(array('allegato' => $this->request['allegato'], 'event' => 'confermaAnnullaRaccolta', 'seq' => $dati['seq'], 'ricnum' => $dati['Proric_rec']['RICNUM']));
                    $open = "location.replace('$href')";
                    $buttonConferma = "<div style=\"margin:10px;display:inline-block;\">
                                              <button style=\"cursor:pointer;\" name=\"confermaDati\" class=\"italsoft-button\" type=\"button\" onclick=\"$open\">
                                                 <i class=\"icon ion-checkmark italsoft-icon\"></i>
            ...
            case 'submitRaccoltaMultipla':
                 ...
                 * LANCIO CLASSI STANDARD
                 */
                if (!$this->callStandardExit(praLibStandardExit::AZIONE_POST_SUBMIT_RACCOLTA, $dati)) {
                    return output::$html_out;
                }
                ...
                if ($datiRDM['Ricite_rec']['ITEMETA']) {
                    ...
                    if ($metadati === false) {
                        output::addAlert('Impossibile estrapolare il template del testo.', 'Errore creazione PDF raccolta dati', 'error');
                        $dati = $this->praLibDati->prendiDati($dati['Proric_rec']['RICNUM'], $this->request['seq']);

                        if (!$this->praLib->AnnullaRaccolta($this->PRAM_DB, $dati, $dati["Ricite_rec"])) {
                            output::addAlert('Impossibile annullare il passo raccolta.', 'Errore annullamento raccolta dati', 'error');
                        }
                    ...
                    if (isset($metadati["TESTOBASEXHTML"])) {
                        ...
                        if (!$pdfPreview) {
                            output::addAlert('Impossibile creare il PDF.', 'Errore creazione PDF raccolta dati', 'error');
                        ...
                        if (!$raccolta_pdfa) {
                            output::addAlert('Impossibile salvare il PDF.', 'Errore creazione PDF raccolta dati', 'error');
            ...
            case 'submitRaccolta':
                ...
                /*
                 * Personalizzazione 21/01/2016 per Apra per salvare il campo COGNOME_NOME invisibile dell'iscrivendo
                 */
                foreach ($raccolta as $campo => $valore) {
                    $Ricdag_rec_tipo = false;
                    if ($Ricdag_rec['DAGTIP'] == "IscrivendiSint") {
                ...
                foreach ($raccolta as $campo => $valore) {
                    ...
                    /*
                     * Condizione TEMPORANEA per procedimento SISMA.
                     * @todo Da normalizzare.
                     */
                    if (strpos($campo, 'CODICE_PRATICA_EDILIZIA_DIGITALE') !== false) {
                        $dati['Proric_rec']['RICRPA'] = '';
                        $dati['Proric_rec']['RICPC'] = '';

                        if ($valore) {
                            if (strpos($valore, '/') !== false) {
                                $valore = substr($valore, 7) . substr($valore, 0, 6);
                            }
                ...
                if ($trovato == true) {
                    ...
                    /*
                     * Attivazione temporanea controntto scuole per Pesaro
                     */
                    $suffix = ITA_DB_SUFFIX;
                    if (file_exists(ITA_SUAP_PATH . "/SUAP_italsoft/ControlliScuola.$suffix.class.php") && $dati['Ricite_rec']['ITECLT'] == "000992") {
                        require_once ITA_SUAP_PATH . "/SUAP_italsoft/ControlliScuola.$suffix.class.php";
                        $obj = new ControlliScuola();
                        $retDoppioni = $obj->ctrDoppioni($raccolta);
                        if ($retDoppioni['Status'] == "-1") {
                            output::addAlert($retDoppioni['Message'], 'Errore', 'error');
            ...
            case "rigeneraDistinta":
                ...
                if (isset($metadati["TESTOBASEXHTML"])) {
                     ...
                     * Creo l'href per fare il download del file
                     */
                    $datiAllegato = $this->getPercorsoAllegato(pathinfo($raccolta_pdfa, PATHINFO_BASENAME), $dati, pathinfo($raccolta_pdfa, PATHINFO_BASENAME));
                    $downloadURI = $this->frontOfficeLib->getDownloadURI($datiAllegato['filepath'], $datiAllegato['filename']);
                    $html->addJSRedirect($downloadURI);
            ...
            case "annullaDistintaComunica":
                ...
                if (!$praLibInfocamere->cancellaPratica()) {
                    return output::$html_out = $this->praErr->parseError(__FILE__, 'E0077', $praLibInfocamere->GetErrMessage(), __CLASS__);
                }

                /*
                 * Ricarico i dati per aggiorniare il disegno
                 */
                $dati = $this->praLibDati->prendiDati($this->request['ricnum'], $this->request['seq']);
                if (!$dati) {
                    return $this->praLibDati->gestioneErrore($this->praLibDati->getErrCode(), $this->praLibDati->getErrMessage(), $this->praErr);
            ...
            case 'accorpaRichiesta':
                ...
                if ($this->request['scollega']) {
                    ...
                    if (!$this->praLib->scollegaDaPraticaUnica($this->PRAM_DB, $this->request['scollega'], false)) {
                        return output::$html_out;
                    }
                } else {
            ...
            case 'onChange':
                $sql = "SELECT 
                        ...
                        FROM
                            RICDAG
                        LEFT OUTER JOIN
                            RICITE ON RICITE.ITEKEY = RICDAG.ITEKEY
                        WHERE
                            RICDAG.DAGNUM = '" . addslashes($this->request['ricnum']) . "'
                        AND
                            RICITE.RICNUM = '" . addslashes($this->request['ricnum']) . "'
                        AND
                            RICITE.ITESEQ = '" . addslashes($this->request['seq']) . "'
                        AND
                            RICDAG.DAGKEY = '" . addslashes($this->request['id']) . "'";
                ...
                if ($objTemplate->checkEseguiEvento('ONCHANGE', $ricdag_rec)) {
                    $objTemplate->eseguiEvento('ONCHANGE', $this->praLibDati->prendiDati($this->request['ricnum'], $this->request['seq']), $ricdag_rec, $this->request);
                }
            ...
            case 'pagamentoMpay':
                ...
                $objFuncPasso = praFunzionePassi::getInstance(praLibStandardExit::FUN_FO_PASSO_MPAY);
                $objFuncPasso->setCodiceIstanza($metaDati['METAOPEFO']['ISTANZA_PAR']);

                $paramTmp = $this->frontOfficeLib->getEnv_config("PAGOPAMPAY_" . $objFuncPasso->getCodiceIstanza(), $this->ITALWEB_DB);
                $clientParam = $this->praLib->GetNormalArrayParam($paramTmp);

                $objFuncPasso->setClientParam($clientParam);
                $risultato = $objFuncPasso->InviaPagamento($dati);
                if ($risultato['Status'] == "-1") {
                    output::$html_out = $this->praErr->parseError(__FILE__, 'E0202', "Errore..Invio Pagamento non andato a buon fine." . $risultato['Message'], __CLASS__);
                    return output::$html_out;
                }
//                require_once(ITA_LIB_PATH . '/itaPHPMPay/itaPHPMPayManager.class.php');
//                $itaMpayManager = itaPHPMPayManager::getInstance($clientParam);
//                $risultato = $itaMpayManager->InviaPagamento($dati);
                break;
            case 'avvenutoPagamento':
                if ($this->request['buffer'] == "") {
                    output::$html_out = $this->praErr->parseError(__FILE__, 'E0200', 'Errore..Pagamento non andato a buon fine ', __CLASS__);
                    return output::$html_out;
                }
                ...
                $objFuncPasso = praFunzionePassi::getInstance(praLibStandardExit::FUN_FO_PASSO_MPAY);
                $objFuncPasso->setCodiceIstanza($metaDati['METAOPEFO']['ISTANZA_PAR']);
                if (!$objFuncPasso->checkDatiPagamento($dati, "MPAY_PID", $this->request['buffer'])) {
                    output::$html_out = $this->praErr->parseError(__FILE__, 'E0201', 'Errore..salvataggio PID non andato a buon fine ', __CLASS__);
                    return output::$html_out;
                }
                ...
                $objFuncPasso->setClientParam($clientParam);
                $risultato = $objFuncPasso->RiceviPagamento($dati, $this->request['buffer']);
                if ($risultato['Status'] == "-1") {
                    output::$html_out = $this->praErr->parseError(__FILE__, 'E0202', "Errore..Ricevi Pagamento non andato a buon fine." . $risultato['Message'], __CLASS__);
                    return output::$html_out;
                }
...
//                $itaMpayManager = itaPHPMPayManager::getInstance($clientParam);
//                $risultato = $itaMpayManager->RiceviPagamento($this->request['buffer']);
                break;
            default:
                break;
        }
    ...
    public function CheckFiereVuote($Anafiere_tab, $raccolta) {
        ...
        foreach ($Anafiere_tab as $Anafiere_rec) {
            foreach ($raccolta['DENOM_FIERA'] as $rowidFiera => $valore) {
                if ($rowidFiera == $Anafiere_rec['ROWID'] && $Anafiere_rec['ROWID'] != 0) {
                    if ($valore == 0) {
    ...
    public function disegnaPagina($dati) {
        ...
        if ($dati['Ricite_rec']['RICSHA2SOST'] != '') {
            ...
            return $praTemplateUploadIntegrazione->GetPagina($dati, array("PRAM_DB" => $this->PRAM_DB, "CLASS" => "praGestPassi", "TIPO" => "UPLOADSUE", "CLASS" => "praGestPassi"));
        } elseif ($dati['Ricite_rec']['ITERICUNI'] != 0) {
            ...
            return $praTemplateRapportoUnico->GetPagina($dati, array("PRAM_DB" => $this->PRAM_DB, "CLASS" => "praGestPassi", 'procedimenti_page' => $this->config['procedimenti_page']));
            ...
            if ($dati['Ricite_rec']['ITEQALLE'] == 1) {
                ...
                return $praTemplateUploadQ->GetPagina($dati, array("PRAM_DB" => $this->PRAM_DB, "CLASS" => "praGestPassi", "TIPO" => "UPLOADSUE", "CLASS" => "praGestPassi"));
            } else {
                ...
                return $praTemplateMultiUpload->GetPagina($dati, array("PRAM_DB" => $this->PRAM_DB, "TIPO" => "UPLOAD", "CLASS" => "praGestPassi"));
            }
            ...
            return $praTemplateUpload->GetPagina($dati, array("PRAM_DB" => $this->PRAM_DB, "TIPO" => "UPLOADSUE", "CLASS" => "praGestPassi"));
            ...
            return $praTemplateInviaMail->GetPagina($dati, array("PRAM_DB" => $this->PRAM_DB, "CLASS" => "praGestPassi",));
            ...
            return $praTemplateInviaInfocamere->GetPagina($dati, array("PRAM_DB" => $this->PRAM_DB, "CLASS" => "praGestPassi",));
        } elseif ($dati['Ricite_rec']['ITEDOW'] != 0) {
            ...
            return $praTemplateDownload->GetPagina($dati, array("PRAM_DB" => $this->PRAM_DB, "CLASS" => "praGestPassi",));
        } elseif ($dati['Ricite_rec']['ITEDRR'] != 0) {
            ...
            return $praTemplateRapporto->GetPagina($dati, array("PRAM_DB" => $this->PRAM_DB, "CLASS" => "praGestPassi",));
        } elseif ($dati['Ricite_rec']['ITEDAT'] != 0) {
            if ($dati['Ricite_rec']['ITERDM'] != 0) {
                if ($dati['Ricite_rec']['ITECUSTOMTML']) {
                    if (strpos($dati['Ricite_rec']['ITECUSTOMTML'], "/") !== false) {
                        list($customFolder, $customTemplate) = explode('/', $dati['Ricite_rec']['ITECUSTOMTML']);
                        $path = ITA_PRATICHE_PATH . "/PRATICHE_italsoft/customClass/$customFolder/$customTemplate.class.php";
                    } else {
                        $customTemplate = $dati['Ricite_rec']['ITECUSTOMTML'];
                        $path = ITA_PRATICHE_PATH . "/PRATICHE_italsoft/$customTemplate.class.php";
                    }
                    require_once $path;
                    ...
                    $returnPraTemplate = $praTemplateRaccoltaMultipla->GetPagina($dati, $this->praLib, array("PRAM_DB" => $this->PRAM_DB, "CLASS" => "praGestPassi",));

                    if (!$this->callCustomClass(praLibCustomClass::AZIONE_POST_RENDER_RACCOLTA, $dati, true)) {
                        return '';
                    }

                    return $returnPraTemplate;
                }
            } else {
                if (!$this->callCustomClass(praLibCustomClass::AZIONE_PRE_RENDER_RACCOLTA, $dati, true) || !($dati = $this->praLibDati->prendiDati($dati['Proric_rec']['RICNUM'], $dati['seq']))) {
                    return '';
                ...
                $returnPraTemplate = $praTemplateRaccolta->GetPagina($dati, array("PRAM_DB" => $this->PRAM_DB, "CLASS" => "praGestPassi",));
            ...
            return $praTemplateDomanda->GetPagina($dati, array("PRAM_DB" => $this->PRAM_DB, "CLASS" => "praGestPassi",));
        } elseif ($dati['Ricite_rec']['ITEDIS'] != 0) {
            ...
            return $praTemplateDistinta->GetPagina($dati, array("PRAM_DB" => $this->PRAM_DB, "TIPO" => "UPLOADSUE", "CLASS" => "praGestPassi",));
            ...
            require_once ITA_PRATICHE_PATH . '/PRATICHE_italsoft/praTemplateDistintaComunica.class.php';
            $praTemplateDistintaComunica = new praTemplateDistintaComunica();
            return $praTemplateDistintaComunica->GetPagina($dati, array("PRAM_DB" => $this->PRAM_DB, "CLASS" => "praGestPassi",));
        }
    }
        ...
        switch ($direzione) {
            ...
            case 'primoRosso':
                ...
                foreach ($dati['Navigatore']['Ricite_tab_new'] as $key => $Ricite_rec) {
                    if (($Ricite_rec['RICOBL'] == 1 || $Ricite_rec['CLTOBL'] == 1) && strpos($dati['Proric_rec']['RICSEQ'], chr(46) . $Ricite_rec['ITESEQ'] . chr(46)) === false) {
    ...
    public function CreaXmlDescrittore($dati) {
         ...
         * nome_adempimento
         */
        $xml .= "   <nome_adempimento>" . $di['datiAdempimento']['nome_adempimento'] . "</nome_adempimento>\r\n";
        /*
         * nome_file_firmato
         */
    ...
    private function getPercorsoAllegato($filename, $dati, $returnFilename = false) {
        if (strpos(pathinfo($filename, PATHINFO_BASENAME), 'rapporto') !== false) {
            ...
            $filepath = $dati['CartellaAllegati'] . "/" . pathinfo($filename, PATHINFO_BASENAME);
        } else if ($dati['Ricite_rec']['ITEDIS'] == 1 || $dati['Ricite_rec']['ITEDAT'] == 1 || $dati['Ricite_rec']['ITEIRE'] == 1 || $dati['Ricite_rec']['ITEUPL'] == 1 || $dati['Ricite_rec']['ITEMLT'] == 1 || $dati['Ricite_rec']['ITEDRR'] == 1) {
            $filepath = $dati['CartellaAllegati'] . "/" . pathinfo($filename, PATHINFO_BASENAME);
